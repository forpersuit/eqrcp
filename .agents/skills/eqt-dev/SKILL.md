...
### 9.3 Receive 模式的多设备 Tus 传输与 CDP 仿真测试指南 (End-to-End Resumable Tus Simulation for Receive Mode)

对于涉及局域网接收端（`Receive` 模式）以及移动端 Tus 协议续传、 done 归档交互、多设备接收状态树重绘的修改，必须通过本套测试指南进行端到端仿真，以验证真实两侧的界面效果和底层逻辑。

#### 9.3.1 本地与多设备自动化测试脚本 (`scripts/e2e-multi-device-simulation.js`)
...
- **核心验证链路**：
  ...
  3. **并发状态树校验 (State Tree Polling)**：脚本以 150ms 频率轮询服务端的 `/send/<token>/status` 接口，打印多设备并发时的 client 状态树，检验 `bytesDone`, `bytesTotal`, `percent`, `state` 是否随断点和续传实时精准更新，防止由于单文件太小未触发 progress 导致的 clientID 未绑定缺陷。
  4. **全局 Keep-Alive 验证**：在启动时必须传入全局标志 `--keep-alive`，防止在多设备并发上传中，单个设备传满设定文件数后服务端立刻自动退出导致其他设备发生 `socket hang up`。

#### 9.3.2 Chrome CDP 页面端到端真机效果仿真
除了 API 级别的脚本，为了对移动端 UI（如添加文件、发送状态、 Done 重载）进行全真操作与视觉交付核对：
...
2. **连接 Chrome MCP 并清理陈旧标签页**：
   ...
   - 在测试导航前，使用 Chrome MCP 的 `list_pages` 查看当前标签。对于残留的历史测试页面（如旧的 `9527` 端口或冲突页面），**必须首先使用 `close_page` 将其关闭**，确保当前的活动 Execution Context 干净且唯一。
3. **导航至接收端动态路由**：
...
未来所有涉及局域网 Share/Receive 模式或 Web 端状态联动的 PR，均需基于此两套仿真测试套路（CLI 模拟与 CDP 页面交互模拟）进行全流程自测和交付成果核对。
