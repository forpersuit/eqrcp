...
> - **K2（低危）测试跳过必须用 `t.Skip` 而非 `t.Log`**：`t.Log` 输出只在失败或 `-v` 时呈现，故 root/`CAP_DAC_OVERRIDE`/非 POSIX ACL 环境下默认 `go test ./...` 仍报 `ok`（探针 D 已实证）——**"显式声明"不等于"可见"**。凡"条件不满足即跳过断言"的分支，一律用 **`t.Skip`**（非 verbose 亦打印 `--- SKIP: <TestName>`），杜绝虚假绿色。
