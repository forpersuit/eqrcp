...
>
> **审查红线（第六轮沉淀 · Rule 12/14/15）**：⑧ **静态类型门禁必须在“本地 pre-commit”、“单套件执行 pretest”与“远端 CI 流水线”三层全覆盖**——仅在 `package.json` 声明 `"typecheck"` 不等于有门禁。目前工程已达成三层无缝闭环：① **本地提交第一现场**：`scripts/deploy-windows-results.sh` 注入 Worker `npm run typecheck`，`git commit` 触发 pre-commit 钩子时本地即刻拦截 TS 错误，严禁带病提交；② **开发调试单套件**：`package.json` 配置 `pretest:cert:offline` 与 `pretest:acme:offline` 原生生命周期钩子，单跑子用例自动前置执行 `tsc --noEmit`，杜绝单套件调试绕过类型检查；③ **远端持续集成兜底**：`.github/workflows/ci.yml` 运行 `npm run test:ci`（链首 `typecheck`）。任何一处改动均在本地与 CI 形成即刻可证伪的强约束闭环。
