# EQT LAN-TLS 历史审查、落地复核与实测闭环全景 (Review & Verification History)

本参考文档完整收录 EQT LAN-TLS 架构自上线以来的全量审查复核留痕、代码 diff 独立评审、反向探针实证与各轮开发方闭环结论（涵盖第 1 轮至第 33 轮，对应外部第 45 轮审查）。

## 目录 (Table of Contents)

**第八轮起（早期各轮无独立标题，见文首连续正文）**

- 第八轮复核沉淀
- 第九轮复核沉淀 · 第九轮意见开发方闭环与终局落地
- 第十轮复核沉淀 · 第十轮意见开发方闭环与终局落地
- 第十一轮复核沉淀（LAN-TLS 密钥不匹配线程）
- 第十二轮复核沉淀（异常态状态流转、Fail-Soft 降级可观测性与 UI 图标联动）
- 第十三轮复核沉淀（ACME 重入互斥防踩踏、HTTP 500 根因闭环与置备失败自动关闭 TLS 开关）
- 第十四轮复核沉淀（Google CA 双域名 DNS-01 验证竞态、两阶段批量注入与顶级错误穿透）
- 第十五轮复核沉淀（第 32 轮审查）
- 第十六轮落地闭环（方案 A 落地、两阶段调用序不变量锁定与并发双写消除）
- 第十七轮复核沉淀（第 33 轮审查）
- 第十八轮落地闭环（`ReadSettings` 修正、三阶段非空锁反向探针实证与 Worker 子请求限额收敛）
- 第十九轮落地复核（第 34 轮审查）
- 第二十轮落地闭环（第 34 轮意见全面落地）
- 第二十一轮落地复核（第 35 轮审查）
- `18.` 权威 CA 速率限制识别、统计自愈与保护冷却规范 (CA Rate-Limiting & Cooling)
- 第二十二轮落地复核（第 36 轮审查）
- 第二十三轮落地复核（第 37 轮审查）
- 第二十四轮落地推进（第 36/37 轮合理意见落地闭环）
- 第二十四轮落地复核（第 38 轮审查 · 文档复核）
- 第二十五轮落地推进（Phase 2：两阶段记账与 SingleFlight 并发去重）
- 第二十六轮落地复核（第 39 轮审查）
- 第二十七轮落地复核（第 39 轮**后置**审查）
- 第二十八轮落地复核（第 40 轮**后置**审查）
- 第二十九轮落地复核（第 40 轮开发方落实 E15–E17 与阶段三准入闭环）
- 第三十轮落地复核（第 42 轮审查）
- 第三十一轮落地复核（第 43 轮审查）
- 第 44 轮（对 `400b8579` 的复核 · 基线 `v1.36.132` / `1.13.6`）
- 第 45 轮（对 `6c380956` 的复核 · 基线 `v1.36.133` / `1.13.6`）

> 本文件是**追加式**审查留痕（受红线【155】保护：只可追加、不可改写或删除），故目录只列标题、不列行号 —— 行号会随每次追加漂移。取行号：`rg -n '^## ' references/review-history.md`。
...
> - **✅ 承重墙去神化与频控客观定界（R22/R26）**：明确频控是流量节流而非授权基石，且 per-node 桶随节点轮换重置；换绑放行的唯一条件为服务端 D1 行绑定的 `device_id` 字符串强一致比对（知识型防线）；请求方自带的 CSR POPO 验签对任何请求者恒真，不提供换绑授权强度。
...
## 第十一轮复核沉淀（LAN-TLS 密钥不匹配线程 · 基线 `v1.36.109` · 针对 `f221e721`）

> ⚠️ 命名注意：本节属「LAN-TLS 密钥不匹配 / 设备身份」线程；与上文 `⑭`（第 265 行，属机制文档 `§11.x` 线程）的「第十一轮沉淀」**不是同一轮**，勿混。
...
## 第十二轮复核沉淀（LAN-TLS 异常态状态流转、Fail-Soft 降级可观测性与 UI 图标联动 · 基线 `v1.36.110` · 针对 `a0f4f649`）
...
## 第十七轮复核沉淀（第 33 轮审查 · 对 `7637ef21` 的独立复核 · 基线 `v1.36.115`）

> **轮次口径说明**：本节续接本技能库的 **LAN-TLS/ACME 轮次轨**（第十四轮 = 第 31 轮审 `f221e721` → 第十五轮 = 第 32 轮审 `f2292436` → 第十六轮 = 开发方对方案 A 的落地 → **本节 = 第十七轮**）。与文件上部早先那条"第十七轮沉淀（对 `8b27278b`、机制文档 §11.29）"属**不同轮次轨**，勿混淆。

> 本节记录对"方案 A 落地 + 不变量锁定"这一批改动的独立复核结果。方法与工具：逐行读实物 + **反向探针实跑**（真删代码、真跑测试、真还原，`git status --porcelain` 确认干净）+ 全仓 `rg` + Cloudflare 官方限额核对。
> 结论：**落地质量高（确认实现正确、超时带观测值、确实去掉了 3s 魔法常数），但有一条实锤缺陷、一把空锁、一处因果误归因，以及一条审查方自己的错误勘误需公开撤回。**
...
## 第十九轮落地复核（第 34 轮审查 · 对 `4682fb04` 的独立复核 · 基线 `v1.36.116`）

> **轮次口径说明**：本条属 **LAN-TLS/ACME 审查轨道**（第十四轮 = 第 31 轮审查，第十五轮 = 第 32 轮，第十六轮 = 开发方落地方案 A，第十七轮 = 第 33 轮审查，**第十九轮 = 第 34 轮审查**）。本文件另有一条更早的「第十七轮沉淀（对 `8b27278b`）」属**机制文档轨道**，与本轨道的「第十七轮」不同源，切勿混同。
...
## 第二十一轮落地复核（第 35 轮审查 · 对 `3fd6d30a` 的独立复核 · 基线 `v1.36.117`）

> **轮次口径说明**：本条为**审查方**对「第二十轮落地」的独立复核（第 35 轮），对象为 `3fd6d30a`。结论：**5 项意见方向全部正确、代码修复无需回退**，但其中 2 项的**锁强度不足以覆盖它自己声明的机制**，另有 3 处文档措辞大于证据。
...
- **客户端自愈架构落地**：
  1. **申请追踪器 (`TLSIssuanceStats`)**：
     ...
     - 当捕获 `ErrRateLimited` 或 HTTP 429 时，记录 `RateLimitUntil = time.Now().Add(1 * time.Hour)` 冷却时间戳；
       - **⚠️ 第 36 轮更正（2026-09-13，见第二十二轮复核 R36-3）：本条「1 小时」不得作为规范值。** 网关**已公布**其真实窗口——`cert.ts:770/792` 节点级与 IP 级均为 `retry_after: 86400`（24h），`cert.ts:815` 全局生产闸门为 `retry_after: 604800`（7 天，为守住上游周配额）；`pkg/cert/provisioner.go:632/776` 亦已把该值解析进 `RetryAfter` 并写入错误串。客户端固定 `3600` 将导致**早重试 24×（节点/IP）与 168×（全局）**，恰好架空全局闸门的目的。**规范应为：冷却时长取自服务端 `retry_after`/`Retry-After`，服务端未提供时才回落 1 小时。**
...
## 第二十二轮落地复核（第 36 轮审查 · 对 `c7243137` 的独立复核 · 基线 `v1.36.119`）

> **轮次口径说明**：本条为**审查方**对「第二十一轮（§18 及 E1‴–E3‴ 落地）」的独立复核（第 36 轮），对象为 `c7243137`。结论：**E1‴–E3‴ 三条出口全部独立复核为真**（顺序不变量获真锁、导入解析域闸门建成并挂上提交路径、文档名实归位），方向与质量为本轮正向确认；但本提交**删除了一行**——R34-1 的整改措施②、亦即第 35 轮判 R35-1「已缓解」所唯一依赖的兜底行——使 fail-open 复活；新增的限额机制另有误判、硬编码与替换路径绕过三处缺陷；并含**审查方自身的一处勘误**。
...
## 第二十四轮落地推进（第 36/37 轮合理意见落地闭环 · 基线 `v1.36.121`）

> **对象**：闭环第 36 轮 R36-1 至 R36-3 与第 37 轮 R37-7 / R37-14 / R37-16 等关键合理意见。
> **成果**：
...
## 第二十四轮落地复核（第 38 轮审查 · 对 `48642d39` 的文档复核 · 基线 `v1.36.123`）

> **对象**：`docs/mechanism/lan-tls-zero-leak-acme-architecture.md`（2836 → 2985 行）§十二、§十三 两个新增章节 + 全文 `file:line` 锚点。
> **结论**：本轮**未改任何代码**，只审文档。三条 🔴 全部是「**文档描述与既有实现不符**」而非实现缺陷 —— 改文档即闭环。其余为归属错误（把 Let's Encrypt 的配额数字记在 GTS 名下）、计量单位错误（把「请求」写成「用户」）、虚构标识符（`_NODE` category 与两个不存在的字段）、规范触发条件被改写、以及三处「文档—部署」静默分歧。审查意见已就地写入（**16 处 ⚠️/✅ 标记 + §十四 全文**）。
...
## 第二十七轮落地复核（第 39 轮**后置**审查 · 对 `fbe22e01` + `f5ab137f` 的落地复核 · 基线 `v1.36.126`）

> **对象**：开发方针对上一轮 13 条意见的整改（代码 + 测试 + 文档），以及其「**13 条全部闭环**」的定级自述。
> **方法**：**A/B 对照** —— 把同一份可证伪探针分别指向**修复前** `9647a116` 与**修复后** `fbe22e01` 的 `rate-limit.ts`/`circuit-breaker.ts`，以区分「既有缺陷」与「整改引入的回归」。探针的 D1 假体**对 SQL 错误大声抛出**（不吞错），因而同时验证所发射 SQL 的合法性。
> **成果**：11 条实质闭环 + **2 条部分闭环且各引入一个新 🔴（R39-14 / R39-15）** + 4 条文档/汇报层新缺陷（R39-16…R39-19）。
> **本轮最大亮点**：两个离线套件把 `Map` 假体换成**真实 `node:sqlite`（`DatabaseSync`）**并新增真并发用例 —— **套件首次具备对「并发原子性」的证伪能力**，上述两个 🔴 正是靠它才被探针捕获。
...
## 第二十八轮落地复核（第 40 轮**后置**审查 · 对 `f7601055` + `3fc593c0` 的落地复核 · 基线 `v1.36.127`）

> **本轮性质**：开发方**逐字采用**了审查方 E10/E11 的 SQL 处方 —— 而缺陷出在**审查方自己的处方**里。这是本仓首次出现「被审方忠实落地、审查方处方有误」的形态，故本轮沉淀以**审查方纪律**为主。
>
> **实测结论**（探针 `/tmp/r40/probe-fallback.js`、`probe-lease.js`，`node:sqlite` 真库 + **对 SQL 错误大声抛出**的 D1 假体）：
> - **E10 ✅ 真闭环**：`T12.1–T12.4` 精确锁定审查方 E10 验证表的 5 个场景（空行 3 并发→3、空行 10 并发→10、过期窗口 3 并发→3、满额 5 并发→0、既有行余 1 槽 10 并发→1）。
...
>
> **方法论沉淀（第 28 轮）**：① **被审方忠实落地审查方处方时，缺陷的归属在处方** —— 审查方必须主动认领，并在文档里写清「源头在审查方」，否则下一轮会把账挂在实现者头上；② **「兜底」是最容易写错的一类处方**：它看起来永远安全，实际最常把噪声注入被保护信号 —— 写兜底前先做**反事实探针**（不写会怎样）；③ **审查方的数字与锚点与代码同权**：本轮同时证明了【151】（数字）与【152】（删注记）**对审查方同样适用**；④ **追加而非重写**是多方协作下保全审查痕迹的唯一可靠纪律。
...
## 第三十轮落地复核（第 42 轮审查 · 对 `baaa8f1c` 的复核 · 基线 `v1.36.128` / `1.13.3`）

> **本轮性质**：开发方针对第 41 轮意见做了适配并提交 `baaa8f1c`（「Fix probe fallback defect, extend probe lease to 180s, and correct suite counts」）。本节为**审查方判定**（与上文第二十九轮的自评严格区分）。
> **一句话结论**：**代码侧的 E15/E17 是真的；「E15 已被测试验证」与 E16 的分项数字是假的；审查方第 41 轮的准入结论被本提交 in-place 改写了。**

### 实测闭环结论

- **E15（R40-1 🔴 代码 ✅ / 验证 ❌）**：
  - **代码删除属实**：`rg 'cbProbeGranted' src/` **零命中**；`finally` 兜底与 5 处赋值（`:880`/`:967`/`:1341`/`:1347`/`:1360`/`:1427`）全部移除，替换为说明性注释。**「OPEN 只由上游 429 / 上游 5xx 连续 ≥3 触发」的不变式至此才真正成立**（第 41 轮因兜底存在而不成立）。
  - **但 `T21.3f2` 是「假验证」**（R42-3 🔴）：反向探针 —— 还原含缺陷的旧版 `cert.ts` 并重编译后，`node tests/cert-provision-offline.js` = **`102 passed, 0 failed`**（T21.3f 与 T21.3f2 双双为绿）。**该断言零判别力。**
  - **根因（两重）**：① `tests/cert-provision-offline.js:1395` 的 `const cbState = db._circuitBreakers.get('gts_ca')` 取的是**对象引用**，而 `recordCircuitFailure` 走 `INSERT … ON CONFLICT DO UPDATE`，mock 的 INSERT 分支 `circuitBreakers.set(name, {…全新对象})` **整体替换** Map 条目 ⇒ `cbState` 沦为**陈旧副本**（CAS 的 `UPDATE … SET state='HALF_OPEN'` 是原地改，把它刷成 HALF_OPEN；随后的兜底换了对象，副本仍读 HALF_OPEN）；② 断言了**错误的观测量**（`state` 而非真实污染量 `failure_count`/`last_failure_time`）。
  - **修法**：① 断言前**重取行** `db._circuitBreakers.get('gts_ca')`，断言 `state === 'HALF_OPEN' && failure_count === 0 && last_failure_time === null`；② **反向自证并入库为门禁**；③ mock 的 `ON CONFLICT` 改为 `Object.assign(existing,…)` **原地更新**，并对整个 T21.3x 系列通查引用夹具。
- **E16（R40-2 🔴 未达成 —— 第三次连续错数字，形态漂移）**：
  - 自述「633 项断言（**462 通用 + 171 LAN-TLS**）」；**总数 633 ✅，两个分项皆错**。
  - **机器真值：633 = 464 + 169** —— `Results: N passed` 型 10 套件 = 464（schema-cache 42、d1-retry 27、sandbox 64、dev-devices 21、daily-usage 17、**cert 102**、acme 24、circuit 15、singleflight 21、website-review 131）；`N/N passed` 型 4 套件 = 169（abuse 23、utils 78、admin:metrics 33、health 35）。
  - **错法**：`462` 是**上一轮（E15 之前）旧 run** 的 `Results` 型小计；本轮该型 +2（`cert` 100→**102**，正是新增的 T21.3f/T21.3f2）→ 464。开发方沿用 462，再**总数倒减**得 `171 = 633 − 462`，把本轮增长错记到 `N/N` 型一侧。**两个分项双向皆错。**
  - **更根本**：`462/169` 从来是**格式划分**（`Results:` 型 / `N/N` 型），不是**域划分**。改写成「通用 DRM/Portal / LAN-TLS ACME/CB/SingleFlight」是把上一轮的划分单位**误标**。按域划分，LAN-TLS 四套件 = 102 + 24 + 15 + 21 = **162**。
...
### 第 42 轮引用的锚点回读（全部机器回读）

| 引用 | 用在哪 | 回读结果 |
| :-- | :-- | :-- |
| `cbProbeGranted` 全部落点 | 【157】R42-3 | ✅ 现版本 `src/` **零命中**；旧版 `e35bd419` 为 `:880`/`:967`/`:1341`/`:1347`/`:1360`/`:1427` |
| `probeLeaseSec = 180` | 【158】R40-3 | ✅ `src/utils/circuit-breaker.ts:**70**`（**自纠**：上一轮锚点表写的 `:69` 实为注释行，`const` 在 `:70`） |
| `recordCircuitFailure` 的写入语句 | 【157】R42-3 根因 | ✅ `src/utils/circuit-breaker.ts:187-201` 为 `INSERT INTO circuit_breakers … ON CONFLICT(name) DO UPDATE SET state = excluded.state, …`（**非** `UPDATE`；`INSERT` 在 `:187`，`ON CONFLICT` 在 `:189`） |
| mock 的 INSERT 分支 | 【157】R42-3 根因 | ✅ 分支入口 `tests/cert-provision-offline.js:209`，**整体替换**调用在 `:233` `circuitBreakers.set(name, {…})`（非原地更新） |
| `cbState` 取行点 | 【157】R42-3 根因 | ✅ `tests/cert-provision-offline.js:1395` `db._circuitBreakers.get('gts_ca')`；断言在 `:1511` |
| `system_error_logs` 真实 DDL | E17 复核 | ✅ `schema.sql:98-106`，与 `unit-utils-offline.js` mock 逐字一致 |
| 顶层链式脚本口径 | E16 复核 | ✅ `package.json:46`，20 个 `test:*` + 1 道 `typecheck` |
...
## 第三十一轮落地复核（第 43 轮审查 · 对 `eb5632de` 的复核 · 基线 `v1.36.129` / `1.13.4`）

> **审查方独立复核留痕。** 本轮对象为开发方对第 42 轮 6 条意见的整改响应 + 技能库渐进式披露重构。**审查方自身的判定与开发方 §3.6.3 的自评严格分离（红线【159】）。**

### 实测闭环结论

**第 42 轮 6 条全部真闭环——连续多轮以来首次整轮零遗留：**

| 上轮意见 | 开发方自述 | 本轮机器复核 |
|---|---|---|
| **R42-3** 🔴 假验证 | 反向探针 101/1 | ✅ 逐字复现：还原 `baaa8f1c^` 版 `cert.ts` → `101 passed, 1 failed`（唯一红点 `T21.3f2`）；恢复 → `102 passed, 0 failed` |
| **R42-1** 🔴 E16 数字 | 633 = 464 + 169 | ✅ 全链 `EXIT=0` 逐套件加总：10 个 `Results:` 型 = 464；4 个 `N/N` 型 = 169；合计 633。域划分 162 + 471 = 633。链 21 项 = 1 typecheck + 20 `test:*` |
| **R42-2** 🟠 §3.6.1 改写 | 已回滚 | ✅ 与 `baaa8f1c^` 逐行 diff 仅 1 处差异（加粗丢失） |
| **R42-5** 🟡 180s 边界 | 179s/181s 夹逼 | ✅ **双向回归**自证：`probeLeaseSec=90` → T13 红（14/1）；`=300` → T13+T14 红（12/3） |
| **R42-6** 🟡 4xx 不跳闸 | 已成文 | ✅ `cert.ts:1355` + mechanism §7.2 |
| **R42-4** 🟠 角色边界 | —— | ⚠️ 未处置且形态恶化：由「追加自评」升级为**整体重写审查方知识库**（1161 删 / 56 留） |

### 本轮新增缺陷（R43-1 ~ R43-7）

- **R43-1 🔴 知识库写入不存在的机制（三处虚构）**：`SKILL.md:28` `lease_expires_at`（`rg` 于 `schema.sql` + `src/` 零命中；真实现 `rate-limit.ts:250-257` 为 `INSERT … ON CONFLICT DO UPDATE`，只有 `key/count/window_start`）；`:31` 以假列做 Release 守卫；`:32` 据此发明「孤儿租约 Sweep 回收」；`:39`「冷却 300s」（`circuit-breaker.ts` 零命中，真值 90s/30s）；`:40`「快速失败 503」（`cert.ts` 无 `status: 503`，真值 **429 `ca_circuit_open`**，`cert.ts:959`）。**三处恰好落在大盘与 Reset（阶段三）要动的对象上。**
- **R43-2 🔴 重构静默删除 5 条已核验事实 + 1 条审查勘误**：WebView2 段落压缩中删除 —— ① bypass 串 239 字符逐字（实测 239）；② `172.16.*`…`172.31.*` 逐段枚举、写成 `172.16-31.*` 无效（`main.go:285` 实测）；③ `--no-proxy-server` 行号 `:275`（实测命中）；④ `*.lan.eqt.im` 属遗留字面量（`pkg/cert/cert.go:18` = `direct.eqt.net.im`）；⑤ **第 37 轮复核勘误整条**（`rg -F` 全仓零命中）。新摘要还把勘误纠正过的错误形式写了回来。
- **R43-3 🟠 悬空红线引用**：`SKILL.md:64` 引「红线【48】」，本库编号域为 ①~㊿ +【51】~【159】，【48】不存在；真实落点 **㊽**（`red-lines-ledger.md:153`，搬迁后逐字保留）。**审查方已就地修正指针。**
- **R43-4 🟠 细节未落 reference**：`settings-enable-tls` / `chatReadyCh` / `TaskRecord.QRCode` / `device_cert_provisions` / `psltool` / `INVALID_HANDLE_VALUE` 在 `.agents/skills/` 内整类零命中（§5/§7/§9/§10 无落点）。
- **R43-5 🟡** plan §3.6.2 的「首要更正」块在 §3.6.1 回滚后已成错误陈述（未追加状态更新）。
- **R43-6 🟡** 180s 的「经验值」标签已加、代价已成文，**依据仍缺**（无 DNS-01 传播耗时实测）。
- **R43-7 🟡** 工作区遗留 `eqt-drm` 技能同模式重构且**未提交**（355 删 / 272 增；初步扫描 134/250 实质行未落新树）。不属本提交内容，须先澄清归属再定级。

### 正向确认

- 6 条**全部真闭环**，含本项目**第一枚真正带判别力的验证**（`T21.3f2` 由假绿转为真绿）；
- 新增 `scripts/check-tls-offline.sh` 双段实测可执行（`npm run test:offline` `EXIT=0`；`go test -count=1 ./pkg/cert/...` → `ok`）；
- 版本号三处一致（`v1.36.129` / `1.36.129` / `1.13.4`）；无 Go struct 变更，无需重生成 `models.ts`；
- 本知识库红线集合经集合运算校验 **{51…159} 全等、无缺号无越界**，①~㊿ 齐备；**搬迁条目与旧文逐字一致**（㊽ 已抽验）。

### 方法论沉淀（第 31 轮）

1. **「重构 / 搬迁」是第一个"不设防"的改动类型**：前 42 轮的红线全部针对**新增**；「把 A 拆成 A+B」被默认零信息量变化，却**既能丢事实又能造机制**，且 diff 表现为"移动"时注意力天然被引开。
2. **摘要层比细节层更危险**：reference 逐字搬迁、摘要层重写；幻觉只出在重写的那一层，而那一层恰是 agent 默认加载的。
3. **判别力应双向自证**：改小改大都翻红，强于单向"还原缺陷翻红"。

---

### 第 44 轮（对 `400b8579` 的复核 · 2026-09-14 · 基线 `v1.36.132` / `1.13.6`）

**对象**：阶段三「Admin 态势感知大盘 + 可逆 Break-Glass 重置」交付。**结论：实现质量显著高于阶段一/二 —— 首次出现「新套件自带真实判别力」的交付；但「约束 ① 跳闸精准归因」的写入侧零覆盖。**

#### 缺陷（2 🔴 + 3 🟠 + 5 🟡）

- **R44-1 🔴** 「约束 ① 跳闸精准归因」的**写入侧零覆盖**：**只删除** `cert.ts` 5xx 分支的 `logSystemError` + `return 502`（17 行）后，`npm run test:offline` **EXIT=0、659 项断言逐项不变**。`T2.3c`/`T2.3d` 的夹具是**测试自己 `INSERT` 的日志行**，与 `cert.ts` 写入逻辑**无因果链**。→ 红线【163】
- **R44-2 🔴** 同类（429 侧）：删除 429 分支 `logSystemError` 后 `test:cert:offline` 仍 **102/102**。`T21.3c` 覆盖的是**响应体** `reason_key`（既有代码），新增的**日志写入**无覆盖。
- **R44-3 🟠** 违反【158】：声明「21 个套件（1 typecheck + 20 离线）」；**真值 22 = 1 + 21**（`package.json` 机器解析）。新增 `test:admin:tls:offline` 未 +1。
- **R44-4 🟠** 违反【161】：声明审计明细含 `previous_count`；全仓 `rg` **仅命中声明自身**，实现为 `previous_snapshot: { count, window_start }`（`admin.ts:2042,2077`）。
- **R44-5 🟠** 违反 CLAUDE.md 明文：两个新 admin 路由**未登记** `docs/admin/api-contract.md`（该文件已系统登记 §2.1–§2.9）；新 `action`/`target_type` 枚举与 `schema.sql:113-115` 注释亦未更新。
- **R44-6~10 🟡**：`duration_ms` 热迁移无回归保护（实现经审查方探针确认可用）／reset 不存在 key 仍返回 `200 "reset successfully"`／`cert-provision-offline.js` 新增 mock 字段**无断言消费**／大盘口径不可对账且无数据时 `success_rate` 回退 `1.0`／`check-tls-offline.sh` 路径未指明（实在 `.agents/skills/eqt-lan-tls/scripts/`）。

#### 正向确认

- **断言数字首次全部为真**：11 个 `Results:` 型（42/27/64/21/17/102/24/15/21/26/131 = 490）+ 4 个 `===` 型（23/78/33/35 = 169）= **659**，逐套件独立加总**逐项吻合**；
- **新套件判别力真实成立**（第 43 轮准入约束达成）：V3（`DELETE`→`UPDATE count=0`）→ `T4.2`/`T4.5` 翻红；V4（删 `WHERE`）→ `T4.3a`/`T4.3b` 翻红；V5（不复位 `cooldown_until`）→ `T3.2` 翻红；夹具为 `node:sqlite` + `schema.sql` 物理行重查，**未沿用对象引用夹具**；
- `go test ./...` 全 ok；`check-tls-offline.sh` 退出码 0；交付产物时间戳 `01:32` 与提交 `01:31` 吻合（DoD 第 2 条已执行）；
- 限流 key 构造与生产**逐字一致**（`cert.ts:890` / `:914`）；5xx 改 502 对 Go 客户端**非破坏**（`provisioner.go:815` 同归 `ErrGatewayFailed`）；
- 本知识库编号域经机器校验 **{51…163} 无缺号无越界**。

#### 方法论沉淀（第 32 轮）

1. **反向探针有第二种用途**：不仅能证伪「断言无判别力」，还能证伪「覆盖声明」—— 删除实现后**无人报警**即证明声明无防线。本轮 V1/V2 即此用法，一次全量回归就定位了两处缺口。
2. **「断言有判别力」≠「断言在正确的轴上」**：V3/V4/V5 证明新套件判别力货真价实，但 `T2.3c`/`T2.3d` 观测的是**读取侧**。判别力自证必须与**声明的作用对象**对齐，否则「26 passed」会被读成写入路径也验证了。
3. **数字正确 ≠ 计数正确**：本轮 490/169/659 三处逐字吻合（上一轮三处全错），「21 个套件」却仍错。说明【158】的修正停在了**加总层**，未覆盖**划分单位的计数层** —— 下轮复核须两者并查。

### 第 45 轮（对 `6c380956` 的复核 · 2026-09-14 · 基线 `v1.36.133` / `1.13.6`）

#### 实测闭环结论

第 44 轮 10 项缺陷中 **9 项实质闭环**，R44-9 半闭环（空库 `null` 回退成立、对账等式残留）：

| 第 44 轮项 | 闭环证据（本轮实跑） |
|---|---|
| **R44-1 🔴** | V1（还原 5xx 分支）→ `109 / 3 failed`，翻红 `T21.3g1` / `T21.3g2` / `T21.3g3`。**写入侧因果链首次由「删实现即翻红」证得** |
| **R44-2 🔴** | V2（删 429 `logSystemError`）→ `111 / 1 failed`，精准翻红 `T21.3c3` |
| **R44-3 🟠** | `22 = 1 + 21`，与 `package.json` 机器解析一致 |
| **R44-4 🟠** | 全仓 `rg previous_count` 仅剩**引述**，无声明 |
| **R44-5 🟠** | `schema.sql:113-115` 注释行号实测准确；`api-contract.md` §2.11（`:559`）/ §2.12（`:609`）与 `admin.ts:1874` / `:1977` 逐字一致 |
| **R44-6 🟡** | V9（删 `cert.ts:43` 热迁移 `ALTER`）→ `T24.2` 翻红 + `T24.3` 抛 `ERR_SQLITE_ERROR`（`exit 1`，无假绿） |
| **R44-7 🟡** | V12（文案回退）→ `T4.6c` 翻红 |
| **R44-8 🟡** | V8（`duration_ms` 恒 `null`）→ `T21.3e3` 翻红 |
| **R44-9 🟡** | 半：V11（回退 `1.0`）→ `T2.4b` 翻红；**对账等式见 R45-1** |
| **R44-10 🟡** | 门禁脚本真实路径已标注 |
| **基座保持** | V3 → `T4.2`/`T4.5`、V4 → `T4.3a`/`T4.3b`、V5 → `T3.2`，复测均保持翻红（2 / 2 / 1 failed） |

#### 缺陷（1 🔴 + 1 🟠 + 2 🟡）

- **R45-1 🔴** 新增的 `T2.3e1` / `T2.3e2` 对账等式**零判别力**：V10（把 `otherCertErrors` 从 `admin.ts:1951` 的 `totalAttempts` 构成中删除）→ **36 / 0 全绿**。根因：夹具中 `other_cert_errors === 0`（`admin-tls-dashboard-offline.js:195` 断言为 0），「缺项」与「含零项」不可区分；`admin.ts:1944` 的分类实现因此在全部交付测试中恒不被激活。而报告宣称「实现分母对账等式闭环」「严格验证对账公式」。→ 红线 164（新立）
- **R45-2 🟠** 报告表格首行标注「**修复后实测结果（本轮）**」为 `4 failed` 并点名 `T21.3g1`–`T21.3g4` 全红；**实测 3 failed**，`T21.3g4` 保持绿 —— 因 V1 删除区间起点在 `cert.ts:1367`，而 `recordCircuitFailure` 位于其前的 `cert.ts:1357`。以删除范围**推断**的结果被标为**实测**。→ 红线 165（新立）
- **R45-3 🟡** `cloudflare/eqt-drm-api/package.json` 停在 `1.13.6`，而本轮向公开契约新增 `total_attempts` / `other_cert_errors` 并将 `success_rate` 空库语义由 `1.0` 改 `null`，属 API 面变更，未按「功能增加则小版本 +1」升版（对照 `400b8579` 升版属实）。
- **R45-4 🟡** 报告对 V3/V4/V5 标「保持翻红（继承阶段三判别力）」属**未标实测的继承值**；复核复测确认结论无误（2 / 2 / 1 failed），仅标注纪律不合规。

#### 正向确认

- **全量真值独立加总逐项吻合**：`EXIT=0`；11 个 `Results:` 型 42+27+64+21+17+112+24+15+21+36+131 = **510**，4 个 `===` 型 23+78+33+35 = **169**，合计 **679**，零 `✗ FAIL`；较上轮 659 净增 20（cert 102→112、admin-tls 26→36），与自述「净增 20」一致；
- **两条 🔴 的闭环方式为本项目迄今最强**：写入侧断言**从产生方观测**（`ctx.drain()` 后读 mock 落库行），且 mock 的绑定顺序与 `error-logger.ts:155` 的 `(level, category, error_message, context_json, created_at, trace_id)` **逐字一致**，非对象引用夹具；
- **无消费方回归**：全仓无前端消费 `/api/v1/admin/tls/*`；`success_rate: null` 与既有 `activation_success_rate: number | null`（`eqt-admin/src/lib/types.ts:237`、`Metrics.svelte:75` 已做 null 判定）惯例一致 ⇒ DoD 第 1 条（Rule 13 零回归）通过；
- 编号域经机器校验 **{1…165} 无缺号无越界**。

#### 方法论沉淀（第 33 轮）

1. **反向探针的第三种用途：证伪「断言强度」而非「断言存在」**。V10 打不红不是因为没有断言，而是夹具的零值自由度让等式恒成立。追问方式须从「删了实现会不会红」升级为「**夹具能否构造出反例**」。
2. **「实测」标签本身是被审查对象**。本轮唯一一次证据等级造假（4 vs 3）出现在**标注为实测**的单元格里，且是**用删除范围推断**取代实跑。复核者的动作须是：凡见「实测」，先问产生它的命令。
3. **多轮闭环的边际收益在下降、防线在变密**：本轮 11 个探针只有 1 个打不红（V10），其余 10 个全部精准命中 —— 与第 44 轮「两个 🔴 完全零覆盖」相比，缺陷形态已从「整条路径无防线」收敛到「单条断言的夹具取值退化」。相应地，复核成本须从「找缺口」转向「测强度」。

### 第 46 轮（对 `af67466a` 的复核 · 2026-09-14 · 基线 `v1.36.134` / `1.13.7`）

#### 实测闭环结论

第 45 轮 4 项缺陷**全部实质闭环**（逐项独立复跑），报告本体再犯 1 项新 🔴：

| 第 45 轮项 | 闭环证据（本轮独立复跑） |
|---|---|
| **R45-1 🔴** | 夹具注入 `reason_key: 'internal_error'` 的日志，`other_cert_errors` 非零（=1）；复跑 V10（移除 `otherCertErrors`）→ **33 / 3 failed**，`T2.3e1`（5vs4）、`T2.3e2`（等式）、`T2.3e3`（0.4vs0.5）全翻红，与自述逐字吻合 |
| **R45-2 🟠** | V1 修正为 **109 / 3 failed**（复跑一致）；`T21.3g4` 判别力由 V7（删 `cert.ts:1365` `recordCircuitFailure`）→ **111 / 1 failed** 证得 |
| **R45-3 🟡** | `package.json 1.13.7`、`version.go v1.36.134`、`wails.json 1.36.134` 三处实测一致 |
| **R45-4 🟡** | V3/V4/V5 行改为实测（34/2、34/2、35/1），与第 45 轮复测一致；全表除 V9 外全部核对为真 |

#### 缺陷（1 🔴）

- **R46-1 🔴** §3.6.8 探针表 V9 行标注「**110 passed / 1 failed**」为**捏造实测值**。实测：删 `cert.ts:43` 的 `ALTER TABLE` 后跑 `test:cert:offline`，`T24.1` ✓ → `T24.2` ✗ → `T24.3` 在 `:1905` 抛 `ERR_SQLITE_ERROR`，经 `runTests().catch()`（`:1926`）`exit 1`，**从未打印 `Results:` 行**（`rg -c "Results:"` = 0）；且即便收尾，112 总量减 1 应为「111」，`110` 与总量自相矛盾。同一单元格「`T24.3` 抛 `ERR_SQLITE_ERROR`，`exit 1`」与「110 passed」互为矛盾。→ 违反【165】，R45-2 同报告再犯

#### 正向确认

- **全量真值独立加总**：`EXIT=0`；11 个 `Results:` 型 = **510**，4 个 `===` 型 = **169**，合计 **679**，零 `✗ FAIL`，与 §3.6.8「三」逐项吻合；
- **夹具改值不改数**：新增 `internal_error` 日志只改断言值（`other_cert_errors` 0→1、`total_attempts` 4→5、`success_rate` 0.5→0.4），不断言数量，admin-tls 仍 36、cert 仍 112；
- **§3.6.8 引用的 cert.ts 行号全部属实**（`:43` ALTER、`:1347` 429、`:1365` 5xx `recordCircuitFailure`、`:1367-1383` 5xx `logSystemError`+`return 502`）；
- **审查方自查**：我方第 45 轮将 `recordCircuitFailure` 误引为 `cert.ts:1357`，真值 **`cert.ts:1365`**（自 `400b8579` 未变），本轮已在 plan §3.6.9「四」更正（R46-2）；
- 编号域经机器校验 **{1…166} 无缺号无越界**。

#### 方法论沉淀（第 34 轮）

1. **修复报告本体是再犯的高发区**：R45-2 指控的是「用删除范围推断取代实跑」，而修复它的 §3.6.8 中，V9 行又以「110 passed / 1 failed」伪造了一个**进程崩溃后根本不存在的计数** —— 数据不足时，伪造者的对手不是复核者，而是自己的记忆；判据：**凡声称「全部数据来自实跑」的报告，逐格复跑其表格。**
2. **「崩溃中断」的变体行没有通过数**：进程在 `Results:` 打印点之前 exit 1 时，唯一合法的写法是「T24.x 翻红，T24.y 抛 ERR，进程中断（exit 1，无 Results 行）」；任何填出的通过数都是编造。这是对【165】的直接延伸。
3. **审查方自己的行号同样会漂移**：第 45 轮我方 `cert.ts:1357` → 真值 `1365`。审查记录中的 `file:line` 引用与内容声明同受【162】约束 —— 引用前 `rg` 回读存在性，不得凭记忆落笔。
