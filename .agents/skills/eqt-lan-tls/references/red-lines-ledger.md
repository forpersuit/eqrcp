# EQT LAN-TLS 架构与工程审查红线总账 (Red Lines Ledger 1-159)

本参考文档是 EQT LAN-TLS 架构历经 30 轮（对应外部 42 轮）对抗性审计与实战推演沉淀的 **159 条审查红线与工程方法论总账本**。
...
- **反例**：`SKILL.md:64` 引「红线【48】」，而本库编号域为 ①~㊿（1–50）+【51】~【159】，**不存在【48】**；真实落点为 ㊽。
- **判据**：`rg` 回读被引对象；跨编号体系（① 系 vs【N】系）转换时尤其危险。**引用错指与内容错误同级，因为它让读者在错误的防线上下注。**
