# EQT LAN-TLS 架构与工程审查红线总账 (Red Lines Ledger 1-166)

本参考文档是 EQT LAN-TLS 架构历经 34 轮（对应外部 46 轮）对抗性审计与实战推演沉淀的 **166 条审查红线与工程方法论总账本**。
...
## 新增红线（第 44 轮 · 对 `400b8579` 的复核）

**【163】声明的写入路径必须有反向探针 —— 断言不得只覆盖读取侧。**
- **触发**：plan / 规格声明某机制「已闭环」，套件全绿，但断言落在**消费**信号的一方，而实现位于**产生**信号的一方。
- **反例**：`400b8579` 声明「约束 ① 跳闸精准归因闭环」——由 `cert.ts` 在 429 / 5xx 分支写入 `reason_key`，大盘按 `reason_key` 分类统计。**只删除写入侧实现**（5xx 分支的 `logSystemError` + `return 502`，17 行）后，`npm run test:offline` **EXIT=0、659 项断言逐项不变**；删 429 分支的 `logSystemError` 后 `test:cert:offline` 仍 102/102。新套件的 `T2.3c` / `T2.3d` 夹具是**测试自己 `INSERT` 的日志行**，与 `cert.ts` 的写入逻辑**无因果链**。
- **为什么比【157】更隐蔽**：【157】的断言**无**判别力（还原缺陷照过），反向探针一戳就破；【163】的断言**有**判别力（删除读取侧实现即翻红），只是**指向的轴错了** —— 读者看到「26 passed」会以为写入路径已一并验证。
- **判据**：对每条 plan 声明的写入路径，**只删除该实现、不动夹具**后必须至少有一条断言翻红；断言观测量须落在**产生**该信号的一方，不得以「手工造出信号 → 验证读取方」替代。**未附反向探针记录的「闭环」声明，按【151】视为数字 / 事实债务。**

---

## 新增红线（第 45 轮 · 对 `6c380956` 的复核）

**【164】恒等式断言的判别力可被夹具的自由度湮没 —— 凡断言「合计 = 各项之和」，夹具中被加总各项必须两两不等且非零。**
- **触发**：断言形如 `A === B + C + D + E`，而夹具令其中某项恒为 `0`（或因等价类退化只取单一取值）。
- **反例**：`6c380956` 新增的 `T2.3e2` 自称「Strict reconciliation: `total_attempts` equals `provisions_success` + all trip reasons」，但**同一夹具**中 `other_cert_errors === 0`（`T2.3d2` 主动断言其为 0）。把 `otherCertErrors` 从 `admin.ts:1951` 的 `totalAttempts` 构成中删除后，`test:admin:tls:offline` **36 passed / 0 failed 全绿** —— 「缺项」与「含零项」数值完全不可区分，等式恒成立。连带后果：`admin.ts:1944` 的 `else { otherCertErrors++ }` 这条分类实现，在全部交付测试中**恒不被激活**。
- **与反向探针的关系**：本缺陷**只在跑反向探针时才暴露**。若仅核对该断言「是否会因字段缺失而翻红」（会），会得出「有判别力」的错误结论 —— 判别力的**强度**取决于夹具能否区分待证命题的两个方向。
- **为什么比「断言无判别力」更隐蔽**：断言字面看起来是**最强**的形态（显式写出等式，非抽样比较），且确实能被字段缺失证伪；弱点不在断言本身，而在**夹具给不出反例**。
- **判据**：恒等式断言的夹具中，被加总各项须**两两不等且非零**，且至少有一项位于「不参与分类」的 `else` 分支上；能构造出「删除任一项后等式仍成立」的夹具，视为该断言无判别力。**恒等式不是证据，能证伪每一项的恒等式才是。**

---

**【165】「实测」是承诺而非修饰 —— 标注「实测」的单元格须由一条列明的命令复现；由删除范围推断出的结果不得标注为「实测」。**
- **触发**：文档 / 报告表格中出现「实测」「measured」「已验证」等证据等级字样。
- **反例**：`6c380956` 的阶段三闭环报告首行标注「**修复后实测结果（本轮）**」为 `4 failed`，并逐一点名四条断言「全部翻红」。实测为 **3 failed**：`T21.3g4` 保持绿 —— 因为该变体的删除区间起点在 `cert.ts:1367`，而被它断言所观测的 `recordCircuitFailure` 位于**之前**的 `cert.ts:1357`，根本不在删除范围内。开发方据「我删了 17 行」推断出「四条应全红」，并把推断写进了标注为「实测」的单元格。
- **为什么必须单独立条**：这类错误的**危害等级高于数字算错**。数字错会被独立加总发现；「实测」标签错会让复核者跳过复跑，等于**用标签换取了免于验证的豁免权**。相反方向的代价是不对称的：一个真跑出来的 `3 failed` 与一个推断的 `4 failed`，在读者眼里证据强度相同，但前者可复现、后者不可。
- **判据**：凡标「实测」处，须同时可指认**产生该结论的命令**；结论若来自「删除范围 + 代码阅读」的推断，须显式写作「推断」或在同一格内附实跑数值。**同一表格中，实跑值与推断值不得共用同一证据等级标签。**

---

## 新增红线（第 46 轮 · 对 `af67466a` 的复核）

**【166】修复报告本体是「实测造假」的再犯高发区 —— 声称修复某类造假/证伪类缺陷的报告，其表格中的每一个「实测」单元格都须被逐格复跑。**
- **触发**：文档 / 报告自称「严禁」「全部数据来自实跑」「彻底消除」某类造假或验证类缺陷，且同一报告内含有表格化的实测数值。
- **反例**：`af67466a` 的 §3.6.8 是第 45 轮 R45-2（「用删除范围推断取代实跑」）的修复报告，第 2 项声称「严禁任何由删除范围推断出的假实测数值，所有数据均来自命令行真实执行」；但其探针表 V9 行标注「**110 passed / 1 failed**」——实测删除 `cert.ts:43` 的 `ALTER TABLE` 后跑 `test:cert:offline`，`T24.1` ✓ → `T24.2` ✗ → `T24.3` 在 `:1905` 抛 `ERR_SQLITE_ERROR`，进程经 `runTests().catch()`（`:1926`）`exit 1`，**从未打印 `Results:` 行**（`rg -c "Results:"` = 0）。「110 passed」在物理上不可能存在：进程在计数打印点（`:1922`）之前崩溃；且即便正常收尾，cert 总量 112 − 1 failed 应为「111 passed / 1 failed」，`110` 与套件总量自相矛盾。同一单元格内「`T24.3` 抛出 `ERR_SQLITE_ERROR`，`exit 1`」与「110 passed」互为矛盾。
- **为什么必须单独立条**：上一轮刚刚为「实测标签」立了【165】，下一轮的修复报告中就再次出现同类伪造 —— 这不是知识点缺失，而是**复核者对「修复报告」的信任度天然高于普通提交**，伪造者利用了这一点。修复报告越声称「全部实跑」，越需要逐格复跑。
- **判据**：凡报告含「修复造假/证伪类缺陷」声明，须**逐格复跑**其表格中**每个**标注「实测」的单元格，不得抽样；进程在 `Results:` 打印点之前 `exit 1` 的变体行，唯一合法写法是「`T24.x` 翻红，`T24.y` 抛 `ERR_SQLITE_ERROR`，进程中断（exit 1，无 `Results:` 行）」，任何填出的通过数一律视为编造。**「崩溃中断」的变体行没有通过数。**
