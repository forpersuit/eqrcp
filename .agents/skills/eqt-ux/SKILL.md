...
- **GUI Workspace & Sidebar Responsive Layout Priority (GUI 主操作区域与侧边栏响应式布局规范)**:
  - **Flex Grid Columns**: Use `grid-template-columns: minmax(0, 1fr) minmax(230px, 300px);` on `.layout` so that the history sidebar (`.side`) shrinks elastically between 230px and 300px on narrower window widths, giving maximum width priority to `.workspace`.
  - **Auto-Height Sidebar & Internal Scroll Isolation**: Set `.side` height to `100%; max-height: 100%; min-height: 0;` to match `.workspace`'s vertical height in `.layout`. Keep `.history` list internal scrolling with `flex: 1; min-height: 0; overflow-y: auto;` so history overflow never expands the sidebar container or squeezes `.workspace`.
  - **Elastic Inner Elements in Workspace**: For elements inside `.workspace` (illustrations, dropzone, QR heroes), use CSS `clamp()` or media queries (`max-height: clamp(...)`, `min-height: clamp(...)`) so they automatically scale down when window height is dragged small (e.g. towards `MinHeight: 640px`), preventing `.workspace` from unnecessarily overflowing and showing scrollbars.
  - **Mobile Breakpoint**: Set the single-column breakpoint to `@media (max-width: 768px)` to ensure the 2-column desktop layout remains active across all Wails draggable window sizes (900px to 1120px+). In single-column mode (<=768px), restrict history height with `max-height: 280px; overflow-y: auto;`.
...
- **Modals & Collapsible Panels Layout**:
  ...
  - **Centered Mobile Modals (QR & Exit Confirmation)**: On mobile viewports, modals like QR code sharing and exit confirmation must be horizontally and vertically centered rather than aligned to the bottom. Apply a 16px safety padding around the screen edge (calculating width as `calc(100% - 32px)` or `calc(100vw - 32px)` up to a max width of `340px`) to align cleanly with the chat shell layout. Enhance the transition with a scale micro-animation (`transform: scale(0.95) -> scale(1)` with opacity change) to achieve a premium feeling.
- **Manual Leave Session Input Blocking**:
  - When the user manually exits the chat session (i.e. `chatSessionStatus !== 'active'`), all composer controls—including the attachment label, textarea, submit button, and hidden file input—must be explicitly set to `disabled` (or visually disabled via `pointer-events: none`). Change the textarea placeholder to "Session ended" or "会话已结束", and apply a muted background wash (`var(--wash)`) to make the inactive state visually clear.
...
- **Mobile Context Menu Positioning & Bottom Input Avoidance**:
  - **Intersection Slice for Long Bubbles**: For extra long bubbles (height exceeding the visible screen/viewport), do NOT position context menus relative to the full bubble bounding rect. Calculate the visible slice intersection between `bubbleRect` and the scroll container viewport (`[vMinTop, vMaxBottom]`), placing the menu cleanly relative to the visible slice.
...
- **Chat iframe Native File Picker & Download Bridge**:
  - The chat page runs inside a Wails-hosted iframe, so `App.svelte` must not call `window.parent.go` or Wails bindings directly.
...
- **Auto-Stop Multi-Device Coordination**:
  ...
  - **Share 模式下自动结束与连接重置容错**：在 `share` (发送下载) 模式下，移动端浏览器（特别是 Android Chrome）在接收完最后一个字节的瞬间，可能会因为主动关闭连接而导致 Go 服务端抛出 `broken pipe` 或 `connection reset by peer` 错误。如果该客户端的总体进度事实上已全部完成（即 `isClientFinished` 返回 `true`），则必须忽略网络连接层报错，将其安全标记为 `"completed"` 以更新 GUI 对勾进度，并允许 `autoStop` 正常进行关机判定。
