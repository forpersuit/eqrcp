...
- **顶栏按钮收敛**: 移动端视口（`<= 820px`）下，顶部右侧仅保留「在线设备数胶囊」（`device-pill`）与「更多选项按钮」（`...`），其余操作（二维码、多语言切换、退出）收进下拉面板（`.more-menu-panel`）。左侧标题徽标 `licenseTier` 为全平台标准展示（桌面端与移动端均呈现于 `EQT` 品牌右侧并支持点击弹窗）；右侧操作区（`head-actions`）仅在免费倒计时生效时展示配额时钟胶囊（`showQuotaPill`），彻底移除此前多余地紧邻设备数胶囊（`device-pill`）展示的重复 tier 胶囊。
