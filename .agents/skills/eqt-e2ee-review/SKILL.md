...
- **CLI 探活外部服务快速降级与内存隔离（第 12 轮 Phase 5 教训）**：CLI 核心为离线局域网场景，启动探测公网 DRM 服务必须设置快速超时（如 800ms~2500ms 适配 WSL 代理/慢网络），绝不可阻塞 5~10s 等待公网超时；高频查询（如状态轮询 snapshot）严禁每次读盘（`v.ReadInConfig`），必须使用内存缓存（`cachedEnableE2EE`）并在写入时刷新。
- **进度计分口径必须按"实际投递字节"而非"磁盘读取字节"（第 13/14 轮教训）**：`_, _ = w.Write(ciphertext)` 丢弃写出字节数与错误时，若改用 `ReadAt` 的返回值计分，连接半途断开仍按整块计入 → 进度虚高。审查发送侧进度上报时核对：计分来源是 `progressResponseWriter` 类按实写字节回调（明文路径 server.go 已有），还是 `ReadAt` 的 `n`；`w.Write` 的错误与短写是否被检查（`writtenBytes < len(ciphertext)` 即不计分并 return）。
- **发送侧进度必须按块去重，防重试/刷新双计导致提前判完成（第 13 轮教训）**：浏览器下载端 `getOrGenerateClientId()` 用 localStorage 持久化复用同一 `client_id`，失败后刷新重下会从头重发 chunk 0..N-1；服务端 `clientProgress` 按 `clientID` 无去重累计时，重复块计分被 `getClientDownloadedAndTotal` clamp 到文件 size → `clientDone >= clientTotal` 提前触发 completed + `markItemDownloaded` + `signalStopAfterStatusGrace`（默认 15s）→ 发送端在剩余块未投递时关停、接收文件截断。接收侧用 `ReceivedChunks` map 按块去重，发送侧必须镜像（`(clientID, fileIndex, chunkIndex)` 三重键）；重试只重投不重计，/meta 刷新再整表清空。**去重 + 写检查 + meta 重置三件套须齐全**。
- **归档（目录/zip）分享进度缺 expectedBytes[-1] 兜底（第 14 轮教训）**：`getClientDownloadedAndTotal` 的 archive 短路分支直接读 `expectedBytes[-1]`，**没有** per-item 分支那样的 `os.Stat` 回退；而 `expectedBytes` 全仓库只在明文发送路径赋值（server.go:2724），E2EE share 路径从不设置 → archive 进度恒返回 (0,0)、永不判完成、发送端不自动停止。修法：meta 里 `expectedBytes[-1] = totalExpectedBytes`，或在 zip 分支补 `os.Stat(s.body.Path)` 兜底（`isClientFinished` 同病）。
- **多文件接收"完成不闪烁"须把文件总数灌进状态（第 14 轮教训）**：E2EE 接收路径从不填充 `cs.Files`（明文 tus 路径才填充，server.go:614-658），故 `len(cs.Files) > 1` 恒 false → 所谓"多文件保持 transferring"分支是死代码；测试若手动注入 `state.Files` 则通过但真实流程无效果。正确做法是把文件总数/序号经协议字段（如 `X-File-Count`/`X-File-Index`）从发送端灌到 `rf` 并据此初始化 `cs.Files`；缺省值（0）须保持向后兼容走 completed。
- **meta 重置与在途 chunk 计分必须原子（第 14 轮教训）**：若 meta 重置与 chunk 计分对两把锁（如 `e2eeShareDeliveredMu`、`clientMutex`）分两次独立临界区持有，刷新页的 meta 删表 与 旧页在途 chunk 的"已投递"标记/计分 可在中间窗口交错：旧请求把标记写进被清空后的新表 → 新会话同块被去重跳过、进度永远差一块到不了 total → 发送端挂死。两处必须**同序嵌套持有**（deliveredMu→clientMutex，一次性加锁释放），消除交错窗口。
- **明文 share 下载是浏览器原生接管，前端无字节级控制（第 16 轮教训）**：明文下载由 `window.location.href` / iframe `?download=1` 触发原生下载（`download.tmpl.html`:1382/:1389/:1404），**没有**可中断的 ReadableStream——凡"暂停=前端中断流、恢复=Range 续传"类设计假设前端有流对象，必须先核对明文下载形态；真断点续传需先改造成前端可控 fetch+Range+本地落盘。另：`download.tmpl.html` 页载即轮询 `/status`（:1877，1200ms，E2EE 期间仍持续，仅 progress 渲染被 `isE2EEDownloadActive` 跳过）→ GUI→移动端 command 通道已具备、响应体未消费 command 字段；而 `upload.tmpl.html`（E2EE receive/tus 上传）**无 /status 轮询**，远程暂停只能靠下一个 chunk POST 命中 423（非即时，最终一致）。
- **单文件 E2EE receive 中间进度回归（第 15 轮教训）**：`X-Total-All-Bytes`（session 总字节）协议字段引入后，`handleE2EEReceiveChunk` 累积逻辑改为 `if rf.SessionTotalBytes > 0 { totalTotal = rf.SessionTotalBytes }` 却**漏了 `totalDone` 兜底**——单文件（`FileCount==1`）时 `state.Files` 只在 `FileCount>1` 分支初始化（保持 nil），循环累加恒得 `totalDone=0`；旧兜底 `totalDone = receivedBytes` 只在 `totalTotal == 0`（未带新 header 的旧客户端）分支执行。结果：新客户端**单文件接收全程 `BytesDone=0, Percent=0`**，直到文件完成跳 100%（最常见的单文件分享场景）。完成路径 `recordCompletedE2EEFile` 因 712-719 `cs.BytesDone = cs.BytesTotal` 不受影响。修法：`SessionTotalBytes > 0` 分支内当 `totalDone == 0` 时补 `totalDone = receivedBytes`。**测试盲区**：场景套件只测了 Share 单文件（发送向）+ 多文件 receive，未测单文件 receive 中间态——审查 receive 进度类提交时务必补断言（首个 chunk 后 `BytesDone>0` 且 Percent 单调递增），用临时测试注入 `FileCount: "1"` + `X-Total-All-Bytes` 即可复现，跑完即删。
- **移动端 E2EE 渐进就绪（边解边存）与全量完成门控时序（第 17 轮教训）**：多文件解密引入边解边存（单文件解密完成即激活该行保存按钮）后，单项交付成功调用 `finalizeDescriptor` 会将该项移出 `sessionPendingFiles`。若后续文件尚未解密入表，待交付表在中间瞬间出现 `sessionPendingFiles.size === 0` 的暂态空表。若按钮完成态收尾仅判断 `size === 0`，会误将整批未完成的后续文件按钮全量提前禁用并打勾 ✓，导致后续文件彻底无法保存。正确做法：全量按钮完成收尾必须增加 `isCompleted && sessionPendingFiles.size === 0` 严格双门控；且完成态条目按钮显露必须按实际拉取的 `filesToFetch` 作用域过滤，杜绝单项下载模式下未下载条目显露保存按钮引发“幽灵交付”误报。
- **移动端局域网 HTTP 真实保存路径与完成态视觉感知重塑（第 18 轮教训）**：①解密 100% 完成后必须强感知重绘卡片主体（大号绿色成功徽标、header-text 与 summary-text 切换为完成态、隐藏等待 Wi-Fi 提示），消除视觉滞后，让用户明确建立“传输已完成，立即保存”的心智；②局域网 HTTP（非安全上下文）下 `navigator.share` 为 undefined，iOS Safari 对 Blob 的 `a[download]` 静默失效；单项/单文件可预览资源（图片、PDF、文本、音视频）必须通过新标签页打开引导 Safari 原生分享【存储到“文件”】，ZIP/二进制提供直接下载与长按指引；③严禁在 fallback 后提前执行 `chunks = null` 销毁内存明文数据，确保数据在页面未关闭前随时可重复打开和保存。
- **打包下载心智回归与自动唤起保存弹窗机制（第 19 轮教训）**：①打包（archive/多文件全部下载）模式下最终原子交付物为单个 ZIP 压缩包；多文件列表仅为包内内容预览，严禁在各行增加冗余的条目保存按钮；②100% 传输解密完成时必须立即自动调用 `saveToDevice()` 尝试第一时间唤起浏览器的原生下载保存弹窗；主按钮统一命名为【Save ZIP Archive】，作为手势过期或权限拦截时的显式兜底；③实测验收必须使用 `results/data` 真实数十至上百 MB 大文件组合（如 44MB + 35MB）进行全链路压测，杜绝仅用小文本测出伪阳性。
- **移动端真机全链路诊断遥测与控制台回传规范（第 20 轮教训）**：桌面端仿真通过不等于真实移动端（特别是真实 iPhone Safari WebKit）行为一致；局域网 HTTP 下权限与行为极易处于黑盒状态；必须在前端建立全生命周期遥测回传（`PAGE_LOAD`、`ACTION`、`E2EE`、`SAVE`、`GLOBAL_ERROR`），通过 `navigator.sendBeacon` 与 `fetch keepalive` 将环境指纹（UA、DPR、`isSecureContext`、`canShare`）、操作轨迹、Worker 解密耗时、保存 API 执行分支及抛出的具体异常（如 `NotAllowedError`、`AbortError`）实时回传至服务端 `/client-log` 并高亮打在终端，消除移动端真机调试盲区。
- **iOS Safari `Unknown` 文件名根因与主按钮原生超链接化（第 22 轮教训）**：①在 iOS Safari WebKit 中，使用 `window.location.href = blobUrl` 重定向下载时，由于 `blob:` 协议仅含 UUID 无文件名，且缺少 HTTP `Content-Disposition` 响应头，Safari 必然将下载任务退化命名为默认字符串 `Unknown`；②彻底移除 `window.location.href = blobUrl`；将主保存按钮改造为具备完整 `role="button"` 的原生 `<a>` 标签；在数据组装完成后，直接将 `href` 绑定为就绪的 `blobUrl`，并将 `download` 属性设置为真实文件名（如 `EQT_SHARE_xxx.zip`）；③用户轻触主按钮时直接放行浏览器的原生超链接下载通道，0ms 延迟在用户即时手势内触发，Safari 官方弹窗 100% 提取并呈现真实文件名，彻底消除 `Unknown`。
- **废除 R20 的 hasIDB 人为拦截与大小文件体验一致性（第 22 轮教训）**：前端切忌因为内部使用了 IndexedDB 分块存储就在交付入口直接 `if (hasIDB) return;` 阻断保存，导致用户点击多次无反应；对于常规文件（≤500MB~800MB）必须放行统一组装交付；并在传输开始时立即向用户展示明确的前置提示（“正在进行端到端加密传输与安全解密，请保持页面开启，传输完成后将自动弹出下载保存提示”），构建确定性心智。
