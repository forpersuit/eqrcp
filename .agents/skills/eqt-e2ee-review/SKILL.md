...
- **iOS Safari WebKit 保存边界与明文组装异常防呆（第 21 轮教训）**：①iOS Safari 在局域网 HTTP 下禁用 Web Share，且对 Blob 的 `<a download>` 完全静默忽略；长时间异步解密后自动触发保存会因缺乏手势激活而被 WebKit 当作未授权弹窗拦截；②组装明文交付文件时严禁无保护直接 `new File([blob], ...)`，部分版本 WebKit 对 TypedArray 参数检验严格易抛 TypeError，应安全回退为原生 `Blob`；③`saveToDevice` 必须覆盖各校验与 catch 分支的全量遥测上报，并在 Fallback 分支实施 `window.location.href = blobUrl` 双重唤起与明确的手动点击保存指引。
