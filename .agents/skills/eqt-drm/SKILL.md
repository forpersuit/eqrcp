...
- **环境操作 SSOT**：测试/生产对照、如何测试、生产清理见 `docs/deploy/environment-runbook.md`（操作前必读）。**wrangler 4.x 已移除 `d1 backup` 子命令**，备份改用 `npx wrangler d1 export <db> --remote --output=...`；生产库任何 DELETE 前先导出备份，删除顺序：`activations` → `license_upgrades` → `unbind_records` → `licenses`（外键约束）。历史教训：2026-07-21 E2E MCP 测试码曾残留生产 D1，任何浏览器 E2E/探针测试只许写 `-test` 库。
