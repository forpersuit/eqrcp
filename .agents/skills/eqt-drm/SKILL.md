...
### 3.6 全量 DRM API E2E 自动化测试套件 (Comprehensive DRM E2E Testing Suite)

为确保云端 Worker API 部署后的功能完备性、日志记录以及防范回归风险，固化了全量 E2E 自动化测试机制：
- **测试路径**：`cloudflare/eqt-drm-api/tests/e2e-drm-test.js`
- **一键测试指令**：在 `cloudflare/eqt-drm-api` 目录下运行 `npm run test:e2e`。
- **自动化测试矩阵（100% 覆盖）**：
  1. **首次设备绑定**：提供有效授权码与设备指纹，测试激活生成 Ed25519 签名密钥及逻辑。
  2. **零消耗重复绑定**：相同设备指纹重新激活，断言返回已有绑定且**不增加设备绑定数/不扣除配额**。
  3. **设备超额绑定**：超过 `max_devices` 上限（如 2 台），绑定第 3 台设备，断言被 403 拦截并返回多语言报错。
  4. **解绑设备**：调用 `/api/v1/user/unbind-device` 正常解绑指定绑定，确认激活列表中物理移除。
  5. **解绑后再绑新设备**：解绑腾出配额后，再次绑定新设备，断言成功绑定。
  6. **滑动 365 天解绑上限**：解绑达到 4 次后发起第 5 次解绑，断言被 403 拦截。
  7. **全量 7 语言 i18n 响应**：测试多语言输入，断言返回匹配的国际化错误卡片。
- **开发准则**：任何 DRM Worker 代码变更部署后，必须运行 `npm run test:e2e` 断言 100% 通过后方可交付。
