...
* **滥用退款黑名单**：滚动 **365 天**内 **已激活过** 的 purchase 退款/拒付 **≥3** → Gate A 邮箱（checkout send/verify）+ Gate B 设备（activate）；**未激活退款不计次**；条款见 `terms.html` / `refund.html`。
* **Dev 注入激活码**：GUI Dev 面板「注入」与 About 兑换均为 `ActivateLicense` → `POST /api/v1/activate`，效果相同，可作高效回归。
