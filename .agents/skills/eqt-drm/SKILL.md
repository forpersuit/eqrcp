...
- **Admin 测试覆盖**：鉴权 fail-closed 拦截、Health 探针及配置徽章、手动发码 `POST /admin/generate`、按 `created_at DESC` 检索 `GET /admin/licenses`、按 `activation_id` 解绑 `POST /admin/unbind`、吊销 404/200 `POST /admin/revoke`、日志查询与清空 `DELETE /admin/error-logs`。
