...
- **动态回暖恢复 (Live Unlocking)**：因为超限设备的心跳并没有因超限而停止，一旦原本占领额度的前两台设备中任意一台关闭页面 8 秒（心跳过期）或点击“停止”释放了额度，超限设备在 1.2 秒的下一次状态轮询中便会恢复为正常等待状态。前端自动触发 `restoreNormalUI()` 将界面无缝还原回可下载状态，无需用户进行任何刷新或二次扫码。
