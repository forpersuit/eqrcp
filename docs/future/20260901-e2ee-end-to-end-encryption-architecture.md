...
```
settings.enableE2EE = false ──────────► 明文传输(灰色「🔓 标准局域网模式」)
...
> 7. **用户体验与端内通知**：三态响应式徽章（绿色 `🔒 E2EE` vs 黄色 `⚠️ 降级传输` vs 灰色 `🔓 标准局域网`），全链路使用端内轻量 Toast/Banner，严禁原生阻塞式 `alert()`；
...
* 免费版或 `enableE2EE=false`:直接走标准明文链路,页面显示灰色「🔓 标准局域网模式」徽章(自愿明文态,非降级,见 §7.3 三态图例)。
...
| **离线行为** | 明文(始终)。 | 明文 + 界面黄色「⚠️ 降级传输」提示(DRM 不可达时)。 |
| **Wi-Fi 防嗅探保护** | 基础随机 Path 混淆。 | 🛡️ 密文级防御(公共/家庭 Wi-Fi 抓包无法解密)。 |
| **安全状态徽章** | 灰色「🔓 标准局域网模式」(自愿明文)。 | 绿色「🔒 E2EE」(加密) / 黄色「⚠️ 降级传输」(离线降级)。 |
