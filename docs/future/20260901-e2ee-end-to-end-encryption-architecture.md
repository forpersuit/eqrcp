...
graph TD
    ...
    Switch -->|"🚫 屏蔽 (Ban)"| GateClose["1. PC 内存标记 sessionBannedClients[ID]=true<br/>2. 静默阻断该设备所有数据面 I/O<br/>3. 不发惊扰弹窗, 仅停止传输"]
    Switch -->|"✅ 解除屏蔽 (Unban)"| GateOpen["1. PC 内存移除 sessionBannedClients[ID]<br/>2. 恢复其请求访问权<br/>3. 强制重置传输状态"]
    
    GateClose --> TransferStop["当前传输立即中断 (HTTP 403 / Socket 挂起)"]
    GateOpen --> ResetRule["⚠️ 安全重置规则: <br/>中途放开的设备不可断点乱接，只能从头 (Chunk 0) 重新发起干净传输"]
```

#### 1. 静默屏蔽 (Silent Ban / 屏蔽设备)
* **设计意图**：最轻量、零惊扰。操作员在 PC 端对某台设备点击 `[🚫 屏蔽]`。
* **执行逻辑**：
  1. PC 服务端在内存中将该设备标记为屏蔽（`sessionBannedClients[clientID] = true`）；
  2. PC 服务端**不再给该设备传输任何数据**（对于下载请求直接返回 `403 Forbidden` 或挂断 TCP，对于上传分块直接拒绝写入）；
  3. **不惊扰客户端**：无需下发大张旗鼓的广播弹窗或破坏性指令，移动端感知为传输暂停/连接中断。

#### 2. 解除屏蔽与从头重置规则 (Reversible Unban & Clean Reset from Scratch)
* **随时可逆**：若操作员误触屏蔽或临时限制后希望恢复，可在 PC GUI 再次点击 `[✅ 允许 / 解除]`，从内存黑名单中移除该设备。
* **从头重置安全准则（核心防错）**：
  - **规则**：若某台设备在**传输进行中**被屏蔽、随后又被放开，系统**严禁在未决的断点碎片处盲目直接恢复传输**（防止 Nonce 状态漂移、分块哈希紊乱或文件写入撕裂）；
  - **行为**：服务端强制重置该客户端的会话传输进度，该设备**只能从头开始重新发起全新传输（Restart from Chunk 0）**；
  - **移动端交互**：移动端点击“重新开始”即可干净地从第 0 块开始完整加密传输，保障 100% 密码学与文件数据完整性。
...
* **桌面 GUI 界面**：
  - 在 Share / Receive 传输面板的设备卡片（`DeviceCard`）上，提供单一、清晰的切换开关或按钮：
    - 正常状态显示：`[ 🚫 屏蔽 ]`（点击后停止对该设备的数据传输）；
    - 已屏蔽状态显示：`[ ✅ 恢复 ]`（点击后允许该设备从头重新发起传输）。
* **CLI 终端交互**：
