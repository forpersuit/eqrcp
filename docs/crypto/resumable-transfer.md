...
>
> 现行 Chat V2 上传走 `/upload` 流式 multipart（非本文 `/api/upload/chunk` 分块协议），下载走 `/files/{id}` 流式 HTTP（非 Range 分块续传）。本文为拟议的断点续传机制；若与加密方案协同落地，必须先阅读配套的 [resumable-e2ee-design.md](./resumable-e2ee-design.md)（含密钥管理、认证与偏移对齐规范）。
...
### 6.1 密码学注意事项（与 E2EE 设计协同时的缺口）

> 本节为审查补充：若本续传机制与 [resumable-e2ee-design.md](./resumable-e2ee-design.md) 的加密方案协同落地，以下两点**必须**一并解决，否则加密形同虚设：

* **分片无认证**：本文 `chunk_index` 与分片内容完全信任客户端，未做签名/校验。任何能触达上传端点的攻击者可写入任意分片、乱序覆盖或注入垃圾块，导致合并后文件损坏或替换。落地时须按 E2EE 文档 §5.2 对每分片附加认证标签（HMAC/AEAD tag）。
* **`file_id` 无内容哈希**：本文以 `文件名 + 大小 + 修改时间` 作快速标识，非内容哈希。恶意方可用相同元数据构造不同内容的 `file_id` 触发"秒传"误判或覆盖。配合分片认证后，宜改为对分片/整体密文做内容派生标识。
* **上传端点需 token 门禁**：`/api/upload/status|chunk|merge` 等接口须与数据面一致做 token 鉴权，禁止匿名写入。
