...
| :--- | :--- | :--- | :--- | :--- |
| **前置 0** | **Phase 0 通配符静默自举先行落地** | **【用户体验托底】**<br>客户端 + CF Worker | 新安全机制涉及外部 PSL 审核（需 1~3 个月），在过渡期内不能让新用户持续遭遇无证书警告与 HTTP 降级。必须先行解决当下断层。 | 新用户下载客户端 1 秒内自动静默获得官方通配符证书，前端黄色警告自动消除，首发即绿锁。 |
...
### Phase 0：当前过渡期“新用户开箱即用”静默自举机制（Auto-Bootstrapping，立即落地）
- **核心目标**：彻底解决新用户首次下载安装 EQT 后因缺少本地证书缓存而提示“未检测到本地有效证书缓存，无证书时将自动降级为 HTTP 传输”的体验断层，实现无需手动执行脚本的零配置全自动开箱即用。
- **交付内容**：
  1. **云端通配符证书分发接口**：在 `lic.eqt.net.im`（Cloudflare Worker）提供安全的证书包拉取能力（`GET /api/v1/cert/wildcard-bundle` 或整合在现有的 `POST /api/v1/device/register` 响应中）；
  2. **客户端异步静默拉取管线**：在客户端启动阶段（通过非阻塞后台 Goroutine），若探针检测到 `!cert.HasValidCertificate()`，自动异步拉取最新通配符证书与私钥并安全写入 `%USERPROFILE%\.config\eqt\certs\`（执行 DACL / 0600 权限收敛）；
  3. **前端状态即时响应**：写入完成后原子刷新内存状态，并通过 Wails 事件总线广播 `eqt:tls-cert-ready`，前端设置面板的黄色警告提示**瞬间自动消除，绿锁就绪**，首次传输即刻享受官方公信 TLS 传输。
