...
> 📌 **措辞一致性复核（2026-09-09）**：上句「绝无可能爆破」「彻底化解」与本章 §4 场景 2 刚收敛的「显著缩小而非完全免疫」属同类**绝对化表述**，宜保持一致口径。防线 2 的安全边界前提是「攻击者无法获取物理屏幕视线」——若同场合近身（如公共 Wi-Fi 下的肩窥/偷拍）则 144 位熵随机会话路径会泄露，故更严谨的定性应为「**大幅抬升攻击门槛（超出主流破解能力）**」，而非绝对保证。建议将「绝无可能爆破…彻底化解」对齐为「破解所需计算量远超现实可行性，显著压缩攻击窗口」。此为措辞精度建议，不改变防线 1（证书 Hostname 强约束）与防线 2（OOB 随机会话路径）在工程上的实质性有效性。
