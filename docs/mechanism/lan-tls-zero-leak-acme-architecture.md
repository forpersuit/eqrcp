...
> ## ⚠️ 全文改判横幅（第 39 轮独立复核 · 2026-09-13 · 基线 `v1.36.125`）
>
> **本文档中一切「全局 40 次 / 7 天熔断」及其端侧表现（`global_rate_limited` / `Retry-After: 604800` / 「7 天冷却期」）的表述均已作废**，不可再作为现役行为引用。该熔断已于 `v1.36.124`（`fc508b5d`）从 `cloudflare/eqt-drm-api/src/routes/cert.ts` **彻底删除**（`global_acme` / `global_rate_limited` / `604800` 于该文件全文零命中，仅余 `cert.ts:931` 的注释文字「Global CA Traffic Smoothing」）。
>
> **受影响的陈旧落点（本轮逐处核对；为免行号随编辑漂移，均以「章节 + 原文引述」定位，可用 `rg -n` 直接检索原文）**：
> - §三.1 时序图：`校验三层频控 (Node 3次/24h, IP 10次/24h, 全局 40次/7d)`；
> - §五.2 三层立体防刷体系：三态图 `[Layer 3: 生产全局熔断兜底] (全局 7天 上限 40 次 …)` 与清单第 3 条 `Layer 3（全局生产熔断）… 40 次置备请求后 … Retry-After: 604800`；
> - §六 当前系统落地实况与代码映射表：`三层立体防刷体系 (Node 3 / IP 10 / 全局 40)` 一行（标 ✅ 真实生效）；
> - §7.1 GTS 官方配额机理真相与历史误区澄清：第 3 点 `「全局 40 次 / 7 天」的真实定位 … 必须严守该 40 次安全缓冲区`；
> - §7.2 多用户并发触发限制墙的系统表现实况：`"retry_after": 604800` 与 `在 604800 秒（7 天）冷却期内再次点击开关将被直接拦截`；
> - §7.3 触碰限制墙后的全生命周期三层解决方案：第二层清单项 `[待闭环] Admin 仪表盘专属配额进度看板（直观显示 当前已用/40）`（分母 40 已不存在）。
> - §7.5 四维立体感知监控体系：三态表**第二维「网关分层拦截」**仍述 `L1 Node 级、L2 IP 级、L3 全局熔断三级拦截` 并标 **【已建成】**（其 `cert.ts:758/780/803` 锚点亦随之失效；L3 应删，锚点应改为 `:886/:909/:931`）；
> - §九 规模化推广与后续演进路线图：`主动开启用户受全局 40次/周 熔断保护`。
>
> **现役替代物**：全局令牌桶（10 req/min、突发容量 5，`cert.ts:931-933`）+ 自适应断路器（`cert.ts:948-950`，**被动反应式** —— 须先吃到上游 429 才跳闸）。**L1（节点 3 次/24h）与 L2（IP 10 次/24h）仍然有效**，但其**并发原子性存在缺陷**，见 [`docs/bugs/2026-09-13-google-ca-wildcard-acme-race-condition-and-tls-state-machine-defect.md`](../bugs/2026-09-13-google-ca-wildcard-acme-race-condition-and-tls-state-machine-defect.md) §十五 R39-1 / R39-2。
>
> **残余风险（须与改判一并知悉）**：删除全局上限后，CA 账户**不再有任何主动上限**，断路器无法回答「首次触墙之前 Google 侧滥用信号是否已形成」。属可接受的设计取舍，但不应以「必须严守 40 次安全缓冲区」的措辞掩盖取舍本身。详见 §7.1 末尾的改判注记。
...
2. **Layer 2（单 IP 级频控）**：防范内网黑客通过脚本本地伪造海量 `node_id` 刷单。单 Client IP 24 小时内最多允许 10 次请求，超限返回 `Retry-After: 86400`；
...
3. **“全局 40 次 / 7 天”的真实定位：EQT 网关的内部安全保险丝**：
   - 网关中的 `cert_provision:global_acme`（40 次 / 7 天，`cert.ts:802`）**并非 Google 官方设定的数字，而是 EQT 团队为了保护主域名绝对不被 CA 封禁而主动设立的保守防御熔断线（Guardrail）**；
   - 该计数对象是**“向云端发起的置备请求次数”**而非独立用户数。多次网络抖动重试均会扣减额度；
   - 在未获得 Google 官方正式书面提额豁免（Rate Limit Exemption）、且主域名未正式合入 Mozilla PSL 之前，必须严守该 40 次安全缓冲区。

> ### ⚠️ 改判注记（第 39 轮复核 · 2026-09-13 · 基线 `v1.36.125`）——本节第 3 点与 §7.2、§7.3 已作废
>
> **上文第 3 点整段（含「必须严守该 40 次安全缓冲区」）在代码层面已不成立。** 提交 `fc508b5d`（`v1.36.124`）已把 `global_acme` / `global_rate_limited` / `604800` 从 `cloudflare/eqt-drm-api/src/routes/cert.ts` **彻底删除**（全文检索零命中，仅余 `cert.ts:931` 的注释文字「Global CA Traffic Smoothing」）。因此：
>
> - **`cert.ts:802` 这个锚点已失效** —— 该行现在的真实内容是 SingleFlight `onConflict` 分支结尾的一个孤立大括号 `        }`，与 40/7d 无关（属「行号漂移后锚点指错位置」，与第 38 轮 R38-8 同类）；
> - **`cert_provision:global_acme` 这一限流键已不存在**，**§7.2「多用户并发触发限制墙的系统表现实况」与 §7.3「三层解决方案」中所有依赖 L3 全局熔断的描述均已过期**，不可再作为现役行为引用；
> - 本文档与 [`docs/plan/lan-tls-google-ca-limit-closure-and-failover-plan.md`](../plan/lan-tls-google-ca-limit-closure-and-failover-plan.md) 就此互斥（后者阶段一明写「从 cert.ts **彻底移除** `global_acme` 40/7d 静态硬编码」），**以代码为准**：本节的「必须严守」不再适用。
>
> **替代物与实际保护面**：L3 被移除后，保护上游 CA 的是 —— ① 节点级 3/24h（`cert.ts:886-887`）与 IP 级 10/24h（`cert.ts:909-910`）；② 令牌桶 10 req/min 平滑、突发上限 5（`cert.ts:931-933`）；③ 自适应断路器 `gts_ca`（`cert.ts:948-950`，三态机 `CLOSED/OPEN/HALF_OPEN`）。
>
> **必须同时交代的残余风险（本注记的重点）**：删掉全局 40/7d 后，CA 账户的**唯一主动上限消失**，断路器是**被动/反应式**的 —— 它必须先吃到上游 429 才会跳闸，因此**无法回答「首次触墙之前 Google 侧的滥用信号是否已经形成」**。这属于可接受的设计取舍（用「首次触墙」换「不再因静态数字集体误伤合法用户」），但不应继续用「必须严守该 40 次安全缓冲区」的措辞掩盖取舍本身。
>
> **本轮另在限流实现中找到 4 条 🔴（并发下限流失效、令牌桶突发反向放量、回滚可侵蚀他人额度、声称的 120 秒 hold 租约不存在）**，逐条证据与可证伪实验见 [`docs/bugs/2026-09-13-google-ca-wildcard-acme-race-condition-and-tls-state-machine-defect.md`](../bugs/2026-09-13-google-ca-wildcard-acme-race-condition-and-tls-state-machine-defect.md) §十五。**§7.5 四维表的第一维**「需改造 `isD1RateLimited` 支持回传当前计数」与本轮 R39-1 的处方（判定与自增合并为单语句 `RETURNING`）应一并实施，避免改两次。

### 7.2 多用户并发触发限制墙的系统表现实况

当多用户并发或批量重装导致触发网关 L3 熔断时，系统的端到端流转表现如下：

> ⚠️ **本节已过期（第 39 轮复核 · 2026-09-13）**：L3 全局 40/7d 熔断已于 `v1.36.124`（`fc508b5d`）从 `cert.ts` 彻底删除，`global_rate_limited` / `Retry-After: 604800` 不再产生。下文描述的拦截与端侧表现**在现役代码中不会发生**；请以 §7.1 末尾的改判注记为准。

1. **云端网关拦截（Worker）**：
   - `cert.ts:803` 拦截请求，直接返回 HTTP 429：
     ```json
     {
       "error": "Global ACME provision ceiling reached (40/7d). Plain HTTP fallback active.",
       "reason_key": "global_rate_limited",
       "retry_after": 604800
     }
     ```
   - 调用 `logRateLimitHit()` 异步记录 D1 `system_error_logs` 表，`category=RATE_LIMIT_CERT_PROVISION_GLOBAL`。
2. **管理后台可观测性（Admin）**：
   - 管理员调用 `GET /api/v1/admin/error-logs?category=RATE_LIMIT_CERT_PROVISION_GLOBAL` 可检索所有被阻断的请求明细与客户端 IP；
   - `GET /api/v1/admin/metrics` 聚合展示 `rate_limit_hits_24h`。
3. **客户端桌面 GUI 表现（Desktop）**：
   ...
   - **气泡提示与动态冷却锁定**：界面弹出系统通知 `触发证书颁发机构频次限制（已自动切换为局域网高速传输，保护期中）`；开关显示警告图标，且在 604800 秒（7 天）冷却期内再次点击开关将被直接拦截，防止无意义重试。

### 7.3 触碰限制墙后的全生命周期三层解决方案（实况审计：已投产 vs 待闭环）

针对 Google CA 限制墙问题，系统规划了三层解决矩阵。下表实事求是地反映每一层的**真实工程闭环现状**：

> ⚠️ **本表第二层一项已过期（第 39 轮复核 · 2026-09-13）**：第二层清单中「`Admin 仪表盘专属配额进度看板（直观显示 当前已用/40）`」的**分母 40 已不存在**（全局 40/7d 熔断已移除，见 §7.1 改判注记）；该项落地时应改为展示令牌桶余量与断路器状态。另：第一层「服务端 Retry-After 动态下发」的措辞与现役实现存在落差 —— **L1/L2 返回的是硬编码 `retry_after: 86400`**（`cert.ts:901/903/924/926`），只有令牌桶与断路器才是真动态值（`cert.ts:941/958`），见复核文档 R39-7。**第三层（Multi-CA / 提额 / PSL）为长期规划，未实现，标注属实。**

```text
┌────────────────────────────────────────────────────────────────────────────────────────┐
│ 第一层：运行时自动恢复与保护机制 【已 100% 投产闭环】                                   │
│   • 服务端 Retry-After 动态下发与客户端冷却锁定（彻底杜绝惊群重试）                    │
...
│   • [待闭环] 第一维前置配额水位感知（70% 黄色预警 / 85% 橙色告警动态计算）            │
│   • [待闭环] 实时告警 Webhook（触达 85% 水位自动推送 Telegram / 企业微信 / 钉钉）       │
│   • [待闭环] Admin 仪表盘专属配额进度看板（直观显示 当前已用/40）                       │
│   • [待闭环] Admin 紧急运维一键清零/解封特定 Node 频控计数器的管理接口                 │
└──────────────────────────────────────────┬─────────────────────────────────────────────┘
                                           │
                                           ▼
┌────────────────────────────────────────────────────────────────────────────────────────┐
...
| :--- | :--- | :--- | :--- |
| **第一维：前置水位感知** | 70% 水位（28/40）Admin 黄色预警；85% 水位（34/40）橙色报警并触发 Webhook 推送运维工单 | **【未建成】** | ❌ 代码中尚无 `0.7/0.85` 水位计算逻辑，无 Telegram/企微 Webhook 脚本；需改造 `isD1RateLimited` 支持回传当前计数 |
| **第二维：网关分层拦截** | L1 Node 级、L2 IP 级、L3 全局熔断三级拦截，并落盘 D1 `system_error_logs` | **【已建成】** | ✅ `cert.ts:758/780/803` 真实生效，`logRateLimitHit()` 真实落库 |
...
│                                                                                        │
│   • 补齐第一维感知: 落地 70%/85% 配额水位计算与实时 Webhook 告警；                     │
