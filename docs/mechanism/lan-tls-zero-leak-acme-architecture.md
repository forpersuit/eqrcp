...
- **根因确认**：`node_id` 初始派生自不可变硬件特征（在未轮换且保留配置时恒定），而客户端私钥是磁盘文件（易失资产）。当私钥因清理缓存、磁盘重装而丢失时，客户端静默重新生钥，向服务端发起请求时因公钥与 D1 不符被判 403 `node_key_mismatch`。若无解绑与自愈机制，该设备将陷入永久锁死。审查员定级为“最重要工程风险”完全成立！
- **分阶段实施路径（注：历史方案 B 已被 v1.36.107/108 的“云端 Fail-Closed + 客户端盐值轮换自愈”终局方案取代）**：
...
#### 11.27 第十六轮独立复核（对 `89882638` · v1.36.89 · 2026-09-11）

审查方法：K1 用 `rg` 核**消费点**（红线 ⑱）；K2 用**独立最小复现**（空包内顶层 skip + 子测试 skip）+ **仓内探针 D′（强制走跳过分支、`-count=1`）**双重实证 Go 的 SKIP 可见性；断言强度用反向探针 C 复验；引文用逐字比对（红线 ⑰）。

##### 一、正面确认表

| 项 | 声称 | 独立核验 | 结论 |
|---|---|---|---|
| K1-a | `main.js` 真实消费 `payload.reason` | `rg` 命中 `main.js:6719/:6721/:6722/:6723`，`reasonKey` → `t(reasonKey)` 查表链路完整 | **真实闭环** |
| K1-b | §11.26 代码块为源码原值 | 文档块与 `main.js:6717-6729` **逐字一致**（仅多一层 markdown 列表缩进，非改写） | **真实闭环（红线 ⑰ 通过）** |
| K2-a | 权限异常抽为独立子测试 | `provisioner_test.go:148` `t.Run("PermissionDenied_UnreadableKey", ...)`；三条跳过路径均为 `t.Skip`/`t.Skipf` | **结构闭环** |
| K2-b | 跳过不影响断言强度 | 探针 C 改写 `provisioner.go:101` 文案 ⇒ 子测试**精准转红**（`--- FAIL: .../PermissionDenied_UnreadableKey`） | **断言完好** |
| 编号 | 本轮无编号漂移 | §11.26 标题「第十五轮演进」与 §11.25「第十五轮独立复核」正确配对 | 属实（错位连发已止） |
| 版本 | `v1.36.89` 双面 | `version.go` = `v1.36.89`、`wails.json` = `1.36.89` | 属实 |
| 零退化 | 套件全绿 | `go test ./pkg/cert` → `ok`；工作树还原后 `git diff --stat` 为空 | 属实 |

##### 二、反向探针记录

- **探针 C（断言强度复验）**：将 `provisioner.go:101` 的 `[WARNING] Failed to read private key at` 改为 `CANNOT-READ-KEY` ⇒ `--- FAIL: TestLoadOrGenerateDeviceKey/PermissionDenied_UnreadableKey (provisioner_test.go:171)` 转红，捕获日志仅余 `[INFO] Generated new ECDSA P-256 private key`。⇒ 加固为子测试后**判别力未退化**。
- **探针 D′（仓内 · 强制跳过分支 · `-count=1`，排除测试缓存）**：把 `if testReadErr == nil` 暂改为 `if true` 以等价模拟 root 下 `ReadFile` 成功 ⇒
  - 默认 `go test ./pkg/cert -run TestLoadOrGenerateDeviceKey -count=1` 输出 **`ok eqt/pkg/cert 0.011s`（无任何 SKIP 标记）**；
  - 加 `-v` 才出现 `--- SKIP: TestLoadOrGenerateDeviceKey/PermissionDenied_UnreadableKey`；
  - 加 `-json` 时 `"Action":"skip"` 计数 = **1**（机器可读）。
- **独立最小复现（空包，规避"是否被测仓缓存误导"的质疑）**：同一测试文件内放一个顶层 `t.Skip` 与一个**子测试** `t.Skip` ⇒ 非 verbose 输出仅 `ok goskip 0.003s`，**两者均不打印**；`-v` 与 `-json` 下均可见（`Action:"skip"`）。⇒ **Go 的非 verbose 输出对"跳过"与"通过"一律不作区分**，与是否子测试无关。
- 探针后源码全部还原，`git diff --stat` 为空，`go test ./pkg/cert` 复绿。

##### 三、本轮新发现（L 系列）

- **L1（中低危 · 断言过强 · 同型第七轮复发，且本轮源头在审查方自身沉淀）**：§11.26 称子测试跳过"被 CI 报告系统与测试框架作为 SKIP 真实捕获，**绝不**伪装成断言通过的虚假绿色"。实证结论：默认 `go test` 仅输出 `ok`，`--- SKIP` 仅在 `-v` 出现，机器可读记录仅在 `-json` 出现 ⇒ **对 CLAUDE.md 明令执行的 `go test ./...` 而言，跳过与通过仍不可区分**，"绝不伪装"不成立。
  - **关键传导路径（新增）**：该错误断言的**源头是上一轮审查写入技能库的 ⑱ 括注**——"（非 verbose 亦打印 `--- SKIP: <TestName>`）"。开发方系**忠实引用审查沉淀**，而非自行夸大。⇒ 错误的审查结论会被下游当成权威依据，**审查方自身的沉淀同样必须经实证方可写入**。
  - **修正后的正确表述**：`t.Skip` 相对 `t.Log` 的真实增益是"在 `-v`/`-json` 下产生**可机读**的 skip 记录（`Action:"skip"`）"，而**非**"默认输出可见"。若要在默认流水线中真正杜绝虚假绿色，CI 必须以 `-json` 采集并对 `Action=="skip"` 设门（或强制 `-v`）。
- **L2（低危 · 命名空间空缺 + 分支不可达）**：`main.js` 通用分支构造 `tls_reason_${payload.reason}`，但 `i18n.js` 中 `tls_reason_*` 词条 **0 条**；后端全仓仅发一种 reason（`app.go:2159` = `node_key_mismatch`），且被特判映射到既有 `tls_key_mismatch_msg`（7 语齐备）。⇒ 该通用分支**当前不可达**，且一旦可达**必缺键**——`t()` 回退 `|| key` 返回字面键名后，落回 `payload.message` 硬编码中文，7 语本地化静默失效。§11.26「进行 i18n 字典路由」对非 `node_key_mismatch` 情形属**名义路由**。建议：注册对应词条，或收敛该分支并在文档注明"当前仅单 reason"。

##### 四、本轮放行结论

- **可放行，无阻断项**：**K1、K2 于源码层真实闭环**——消费点确已接线（`rg` 命中四处）、跳过路径确已改 `t.Skip`/`t.Skipf`、断言强度经探针 C 保留；§11.26 引文逐字属实（红线 ⑰ 通过）；编号配对正确；版本双面一致；`go test ./pkg/cert` 全绿、零退化。
- **须收敛项**：**L1**（§11.26「绝不伪装成虚假绿色」对默认 `go test` 不成立；**并须同步修正技能库 ⑱ 的同源错误括注**）、**L2**（`tls_reason_*` 词条空缺，通用分支不可达且必缺键）。
...
#### 5. Google CA 限制墙对 EQT 规模化推广的真实影响深度评估 (Business Impact & Scale-Up Roadmap)

##### (1) 产品基础盘：核心文件传输功能 100% 免疫，推广零阻塞
- **第一性原理事实**：EQT 的核心杀手级功能是**局域网零配置、跨平台极速文件互传与聊天**；
- **默认配置策略**：软件出厂与首次安装时，局域网 TLS 加密开关**默认处于关闭状态（`enableTLS: false`）**；
- **传输模式**：默认直接走高速局域网 IP 直连（如 `http://192.168.1.5:8080`），二维码直接编码物理 IP，**不产生任何外部 DNS 解析，不消耗任何外部 CA 证书，不与外部云端网关通信**；
- **结论**：**无论推广给 1,000 个用户还是 1,000,000 个用户，只要使用默认的标准互传模式，Google CA 的限制墙对核心业务的影响为绝对的 0！**

##### (2) 增值安全盘：主动开启 LAN-TLS 用户的体感与受限边界
当推广吸引来注重极高安全隐私、或在公司/公共 Wi-Fi 环境下主动进入设置开启「局域网 TLS 加密传输 (LAN-TLS Encryption)」的高阶用户时：
- **现行工程阈值下的物理上限**：
...
- **撞墙后的用户真实体感（Fail-Soft 保护）**：
  ...
  - **视觉状态呈现**：开关旁显示黄色警示图标，鼠标悬浮显示冷却与限额说明，传输卡片正常显示 `HTTP (降级明文)`，依然能以最高物理网速完成局域网传输；
  - **用户心理预期管理**：对于普通用户而言，可能产生疑惑“为什么提示频次限制？是不是服务不可用？”因此在推广放量前，必须对该配额与架构做出针对性解耦。

##### (3) 规模化推广的三阶段应对与演进路线图

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    EQT 规模化推广阶段与 CA 限制墙应对矩阵                     │
├──────────────┬──────────────────┬─────────────────┬─────────────────────────┤
│   推广阶段   │   并发用户规模   │   当前瓶颈状态  │       推荐工程解法      │
├──────────────┼──────────────────┼─────────────────┼─────────────────────────┤
│ 阶段一：种子 │ DAU < 1,000      │ 现行 40次/周    │ 1. 默认 HTTP 明文保持   │
│ (当前阶段)   │ TLS 用户 < 40/周 │ 满足极早期内测  │ 2. 动态 Retry-After 防刷 │
├──────────────┼──────────────────┼─────────────────┼─────────────────────────┤
│ 阶段二：成长 │ DAU 1,000~50,000 │ 40次/周将被击穿 │ 1. 提交 Google GTS 提额 │
│ (公开推广期) │ TLS 用户 > 100/周│ 出现 429 告警   │ 2. 启用多 CA 轮换(GTS+LE)│
│              │                  │                 │ 3. 付费权益隔离(PLUS/PRO)│
├──────────────┼──────────────────┼─────────────────┼─────────────────────────┤
│ 阶段三：成熟 │ DAU > 50,000     │ 单域名上限击穿  │ 1. 泛域名集中置备 O(1)  │
...
2. **阶段二（公开推广与商业化启动，DAU 1,000 ~ 50,000）**：
   - **行动 1（零代码投入，官方提额）**：由运维团队向 Google Cloud 提交 Google Trust Services Rate Limit Exemption 申请，说明 EQT 客户端的分布式 P2P 场景，将 `eqt.net.im` 提升至 10,000+ 张/周；
   - **行动 2（多 CA 灾备生效）**：正式激活 Worker 中 GTS 与 Let's Encrypt / ZeroSSL 的动态轮换，配额叠加翻倍；
   - **行动 3（商业特权化分流）**：将「公信级局域网 TLS 泛域名证书置备」明确定义为 PLUS / PRO 付费版本专享高级权益，普通免费版专注极速纯局域网明文传输，天然形成配额保护屏障并助推软件变现。
3. **阶段三（规模化爆发期，DAU > 50,000）**：
   - **架构根本蜕变（方案 C）**：实施“中心申请共享泛域名证书 + 本地会话 ECDH 加密”。不论全网有多少万台设备使用，云端每周恒定仅向 Google CA 申请 1 张证书，彻底终结公共 CA 的限制墙威胁，实现无限扩展。

---

#### 6. 深度推演：为什么现阶段坚决摒弃“阶段三（集中通配符共享）”？三大密码学与物理体验死穴

在评估多用户并发与 CA 配额瓶颈时，直觉上最容易想到的“捷径”是阶段三（即方案 C）：*“既然申请海量单机证书会触碰 Google CA 限制墙，为什么不全网共用 1 张 `*.direct.eqt.net.im` 通配符证书，将配额消耗降为常数级 $O(1)$？”*

从**第一性原理**审视，阶段三是用牺牲**核心密码学安全**与**移动端大文件物理体验**为代价换取配额节省，存在三大不可逾越的物理与工程死穴：

##### (1) 密码学死穴：私钥出机退化为“全网共享秘密”，同网内中间人窃听（MITM）毫无防备
- **公钥绑定律不可违背**：在 WebPKI (X.509) 规范下，一张证书在数学上强绑定单一公私钥对；
- 若全网设备共享同一张通配符证书，意味着私钥（`privkey.pem`）必须下发并保存在每一个用户的客户端本地；
- **局域网伪造与监听**：在办公室、星巴克或校园公共 Wi-Fi 下，任何提取了该私钥的恶意用户，只需发起一次 ARP/DNS 欺骗，利用这把合法的通配符私钥伪装成目标机器应答。**受害者的手机 Safari 不仅毫无警觉，还会亮起权威公信绿锁 🔒**！用户传输的敏感数据、私人照片和聊天记录在合法绿锁的掩护下被完全窃听与篡改。

##### (2) 连坐吊销死穴：单机泄露引发全网集体瘫痪（Blast Radius 无限大）
- 私钥分散在不可控的公网海量客户端磁盘中；
- 只要全球有任意一台安装了 EQT 的用户 PC 感染木马，或被安全研究员逆向提取后公开在 GitHub / 扫描平台上；
...
- 若为了防御同网窃听而在应用层再套一层端到端（E2EE）ECDH/AES-GCM 加密：
  ...
  - 必须由前端 JavaScript / WebAssembly 逐字节进行流式解密；
  - **移动端内存极限硬墙**：在 iOS Safari 极度严苛的单 Tab 内存管控下（通常上限仅 1GB~1.5GB），传输 20GB~100GB 的 4K 蓝光高清电影或巨型数据集时，前端解密必然引发**严重的垃圾回收滞后与 WebKit OOM 闪退崩溃**；
  - 浏览器原生的多线程下载、断点续传以及内核 C++ 直接流式写盘（零内存开销、跑满千兆网速）的核心卖点荡然无存！

##### (4) 工程结论：拒绝饮鸩止渴，坚持单机单私钥正途
- 工业级标杆 **Tailscale** 的成功实践证明：“本地私钥自生成（永不出机）+ 专属动态子域 + ACME 自动化置备”是兼顾“真正零泄露”与“原生免装 App 绿锁”的唯一正确解；
- 阶段三属于严重的**过早优化与技术倒退**。在当前及可预见的发展期，坚决不采用阶段三。

---

#### 7. “得知触墙”的极高战略重要性与四维立体感知体系深度设计
...
```
┌────────────────────────────────────────────────────────────────────────────────┐
│                   多用户 LAN-TLS 配额触墙四维立体感知与告警拓扑                   │
├────────────────────────────────────────────────────────────────────────────────┤
│                                                                                │
│  [第一维: 提前量水位感知]                                                       │
│  D1 周期聚合近 7 天签发量 ──► 70% 水位 (28/40) ──► 触发 Admin 预警通知/Webhook  │
│                           ──► 85% 水位 (34/40) ──► 标红紧急待办：提额/启动备用CA│
│                                                                                │
│  [第二维: 云端网关拦截感知]                                                     │
│  POST /api/v1/cert/provision                                                   │
│     ├── L1 节点级 (3次/24h) ──► 429 {reason: rate_limited, retry_after: 86400} │
│     ├── L2 IP 级 (10次/24h) ──► 429 {reason: ip_rate_limited, retry_after:...}│
│     └── L3 全局 (40次/7天)  ──► 429 {reason: global_rate_limited, 604800}     │
│           └── 异步写入 D1 system_error_logs (category: RATE_LIMIT_*)           │
│                                                                                │
│  [第三维: 上游 Google CA 穿透感知]                                              │
│  穿透网关 ──► GTS ACME 429 urn:ietf:params:acme:error:rateLimited              │
│     └── 原文捕获 ──► D1 system_error_logs (category: CERT_PROVISION_ERROR)     │
│                                                                                │
│  [第四维: 端侧无恐慌状态流转]                                                   │
│  客户端接收 429 ──► 锁定 IsRateLimitedActive + 继承真实 Retry-After            │
│                 ──► Fail-Soft: 自动弹回关闭态，任务卡片展示 HTTP(降级明文)       │
│                 ──► 气泡通知与开关旁黄色警示图标 (禁用无谓重试，杜绝惊群)       │
└────────────────────────────────────────────────────────────────────────────────┘
```

##### (1) 第一维：前置配额水位感知（Proactive Watermark Sensing）
- **核心原则**：绝不能等到第 40 张或 Google CA 报错时才“后知后觉”，必须设立分级预警缓冲带；
- **分级策略**：
  - **70% 水位（黄色预警，28/40）**：在 Admin 仪表盘将配额进度条渲染为黄色，自动记录 `WARN` 级审计日志，提醒运维团队向 Google Cloud 提交 GTS 提额申请；
  - **85% 水位（橙色警报，34/40）**：系统在监控大盘置顶警告，可通过配置的 Webhook（如钉钉/企微/Telegram/邮件）向管理员推送应急工单，准备无缝切换至备用 CA（Let's Encrypt / ZeroSSL）；
  - **100% 水位（红色熔断，40/40）**：触发网关 L3 保护，返回 `global_rate_limited`，拦截请求穿透至上游，保护母域免受 Google CA 惩罚性封禁。

##### (2) 第二维：云端网关拦截分层感知（Gateway Multi-Tier Observability）
- **L1 节点级、L2 IP 级与 L3 全局硬熔断**清晰解耦；
- 每次触发拦截，`logRateLimitHit()` 均记录详细上下文：
  - `category`: `RATE_LIMIT_CERT_PROVISION_GLOBAL` / `RATE_LIMIT_CERT_PROVISION_NODE` / `RATE_LIMIT_CERT_PROVISION_IP`
...
- 若请求穿透网关但在与 Google Trust Services 通信时收到异常（如主域名被 Google 官方限制）：
  ...
  - 将包含 Google 官方返回的原文（例如 `"too many certificates already issued for 'eqt.net.im'"`）记录在 `system_error_logs` 表（`category: CERT_PROVISION_ERROR`）；
  - 管理员在后台直接查看 Google 官方错误原因，为向 Google 提交提额申诉提供 100% 确凿的凭据。

##### (4) 第四维：端侧体验与状态闭环感知（Client Fail-Soft State Loop）
- 严格遵循**无侵入（Anti-Alert）原则**，严禁弹出系统级 alert 对话框；
