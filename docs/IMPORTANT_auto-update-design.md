...
| 模式 | 描述 | 适用场景 |
| :--- | :--- | :--- |
| `silent`<br>(**默认**) | **静默安装**。后台检测并自动下载新版本。下载完成后完全不打扰用户，在用户下次启动软件时无感自动应用最新版本。 | 追求完全无感自动升级的用户。 |
| `download` | **后台下载并通知确认**。后台检测到新版本时，自动在空闲时间下载更新包。下载完毕后提示“新版本已就绪”，且**重启不会自动更新**，必须由用户在界面显式点击“重启并应用更新”按钮才执行安装。 | 需要完全掌控升级时机、随时主动点击升级的用户。 |
