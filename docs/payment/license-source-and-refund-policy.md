...
Portal → `POST /api/v1/user/refund` → Paddle adjustment → `status=revoked` + `revoked_at` → 邮件 → 客户端 `/verify` 降级。
...
```text
Paddle transaction.refunded / 订阅取消
...
- [ ] Admin UI 选择 source=promo 与兑换窗表单  
- [ ] purchase 年付与 `duration_days`/`expires_at` 语义彻底分离（双重过期 SSOT）  
- [ ] 结账页显式勾选/链接强化  

### P2

- [ ] `revoke_reason`（refund / chargeback / admin / expired）  
- [ ] Pro 订阅与中继流量计量  
