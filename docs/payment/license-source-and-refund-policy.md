...
4. **条款与代码一致**：黑名单、退款次数限制必须在 **Terms** 中披露。2026-08-20 起，`refund.html` 按 Paddle 域名审核要求改为**无条件 14 天全额退款**（不再含任何限定条件/例外），黑名单披露统一移至条款「合理使用与反滥用」。
...
| 披露 | **Terms of Use「合理使用与反滥用」条款**（2026-08-20 起；退款政策按 Paddle 域名审核要求改为无条件 14 天，不再写黑名单）；结账提示链到退款政策 |
...
**2026-08-20 调整**（Paddle 域名审核要求退款政策无「限定条件/例外」）：黑名单披露从 `refund.html` 移到 `terms.html`「合理使用与反滥用」；`refund.html` 改为与 Paddle 一致的无条件 14 天全额退款。

合适位置：

1. **`terms.html`** — 法律约束主文（接受条款即接受限制）。新增「Seller & Legal Entity」（法定公司名 `Huiai Technology (Xinzhou) Co., Ltd.`）与「合理使用与反滥用」（承接黑名单披露）
2. **`refund.html`** — 退款政策：14 天全额退款（付费订单）+ 订阅取消 + 由 Paddle 处理；**不含任何限定条件/例外**
3. **Pricing 结账** — 链到 Terms / Refund Policy（「支付即表示同意…」）

文案要点：

- 14 天购买冷静期（付费订单，**无条件全额退款**，2026-08-20 起）
- **条款**中披露：滚动 365 天内，同一邮箱或设备因退款/拒付导致的「已激活」授权吊销达到 3 次，EQT 可拦截后续购买/激活（反滥用，非退款资格限制）
