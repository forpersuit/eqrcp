...
5. **已验证 live 状态（2026-08-19，登录 live Dashboard 核对）**：
   ...
   - ⚠️ **域名审核**：`eqt.net.im` 处于 Pending（**该审批即覆盖 www**——Paddle 域名输入框明确拒绝 `www.` 前缀，勿再单独提交 www）。2026-08-20 实测排查 Pending 根因：① onboarding「02 Verify your account」仍 **Not started**（Paddle 需先验证业务才放行真实收款，最可能的闸门）；② 此前 apex `eqt.net.im` 无任何 Web 记录，审核抓取不到站点内容——当天已配 apex→www 301，现审核可跟随跳转到完整合规站点。
   - 📧 **2026-08-20 用户完成验证提交 → Paddle 邮件临时批准**：`eqt.net.im` **provisionally approved**；payout 前需整改 2 项（均已改并部署）——① 退款政策含限定条件/例外 → 改为**无条件 14 天全额退款**（refund.html 移除设备/技术条件、不可退款码例外、365 天限制）；② 条款缺法定公司名 → terms.html 新增「Seller & Legal Entity」（`Huiai Technology (Xinzhou) Co., Ltd.`）与「合理使用与反滥用」条款（365 天黑名单披露移至此处，Paddle 最低条款要求有 Misuse 条款）。pricing.html 结账提示同步去掉 365 表述。过审后把 `default_checkout_url` 设为 `https://www.eqt.net.im/pricing`。
...
6. **第二轮推进（2026-08-20，live 上线收尾）✅**：
   - ✅ **域名 `eqt.net.im` 已 Approved**（Dashboard 域名状态从 Pending/provisional → **Approved**，覆盖 www）。
