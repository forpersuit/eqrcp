...
sequenceDiagram
    ...
    alt 未曾绑定 (首绑路径)
        ...
        Worker->>Worker: 校验 Header 中的 deviceIdHeader (头名 'n')
...
| :--- | :--- | :--- | :--- | :--- |
| **7.2.1 驳回匿名轮换** | 🔴 高 | `node_id` 仅 48-bit 且公开，匿名自动覆盖会导致 DoS 抢占与配额耗尽 | **完全采纳**。驳回原 §5 匿名覆盖提案，确立基于既有 `n` 头（`device_id`）的硬件身份认证式换绑架构，配以 30 天换绑冷却期。 | 理论架构已确立，待后续排期实施 |
...
| **7.2.4 90天有效期澄清** | 🟠 中 | 云端无 ACME 吊销端点，本地证书在 90 天有效期内依然具有密码学合法性 | **完全采纳**。已在 §3.2 场景 3 纠偏为“旧证书停止续期、旧公钥在云端被新公钥覆盖并失效，私钥泄露威胁窗口为至多 90 天”。 | ✅ 本文已修正 |
...
### 10.2 针对 S2：锁定既有请求头 `n`，规避存量版本回归（Rule 13）

- **坚决否定新造请求头**：
  - 审查员指出的关键事实：客户端已经在 `pkg/cert/provisioner.go:375` 通过请求头 `n` 发送了 `opts.DeviceID`，且 Worker 已在 `cert.ts:591` 读取并赋给 `deviceIdHeader`，并在 `cert.ts:896` 写入了 `node_public_keys.device_id`。
  - 若冒进引入 `X-EQT-Device-ID`，不仅是重复造轮子，更会导致存量客户端（仍发 `n` 头）在云端被误判为「未带 Device-ID」而遭到 403 阻断，引发严重的存量回归破坏（违反 Rule 13）。
- **终局实现规格**：
  - **保持协议头完全不变**：沿用既有请求头名 `n`；
  - **补齐最后 33.3% 闭环逻辑**：在 `cert.ts:869-878` 的 mismatch 分支内，将现有的：
    ...
    ```
    在 mismatch 分支中比对已绑定的 `device_id` 与当前的 `deviceIdHeader`（即请求头 `n`）。仅在比对一致且满足冷却期时，才允许执行更新。
...
1. **首绑分支前置门禁 (Mandatory Device Registration for First Binding)**：
   ...
   - Worker 在 `cert.ts` 执行首绑 `INSERT` 前，不仅校验请求头 `n` 非空，还必须通过 D1 校验该 `device_id` 确实存在于 `device_registry` 中且状态正常；
2. **设备注册频控下沉至 D1 / KV**：
...
云端在处理公钥不一致的换绑请求时，不采用单一的 `device_id` 等值比较，而是采用**阶梯式多维证据链**：

```mermaid
flowchart TD
    Start[检测到公钥不一致 mismatch] --> CheckCooling{是否处于换绑冷却期内?<br/>如单节点30天}
    CheckCooling -- 是 --> Reject429[429 换绑过于频繁]
    CheckCooling -- 否 --> Vector1{维度 1: 凭据直接比对<br/>req.n === bound.device_id ?}
    
    Vector1 -- 一致 --> AllowRebind[校验通过: 允许自愈换绑]
    
    Vector1 -- 不一致 (可能换盘或重铸ID) --> Vector2{维度 2: 主板硬件拓扑校验<br/>req.uuid_hash 存在且等于<br/>原绑定 device_id 的历史 uuid_hash ?}
    Vector2 -- 匹配 --> AllowRebind
    
    Vector2 -- 不匹配 (主板亦更换) --> Vector3{维度 3: 授权凭据持有证明<br/>请求携带合法未过期的 .lic 授权证明 ?}
    Vector3 -- 有效 --> AllowRebind
    
    Vector3 -- 无凭据 --> Reject403[403 拒绝未授权换绑]
```

- **证据链优势**：
  1. **日常重装系统/误删目录**：硬件完全未变，命中【维度 1】，秒级自愈；
  2. **更换系统盘/CPU指纹常空**：`device_id` 虽变，但主板 UUID 历史匹配，命中【维度 2】，平滑自愈；
  3. **整机迁移/主板更换**：用户导入原有的付费许可证凭据，命中【维度 3】，平滑自愈；
  4. **局域网旁观攻击者**：三者皆无，100% 被拒之门外，且受到单节点换绑冷却保护。
...
| **S2** | 🟠 中高 | 请求头已存在（名为 `n`），通道已完成 66.7%，不应改名为 `X-EQT-Device-ID` 避免破坏存量兼容 | **完全采纳**。坚决保持既有请求头 `n`，规避版本回归；实现规格收敛为在 `cert.ts:869` mismatch 分支执行等值比较。 | ✅ 规格已锁定 |
| **S3** | 🔴 高 | 「彻底切断全局配额耗尽」不成立，首绑分支无任何门禁，注册端点可无限刷号 | **深度反思并纠偏**。撤回绝对化表述，提出首绑 `device_id` 存在性校验 + D1 持久化注册频控 + ACME 原子预扣的综合防护架构。 | ✅ 方案已重构 |
| **S4** | 🟠 中 | 方案以 `device_id` 保密性为前提，但全文未确立该不变量，且 About 面板明文展示 | **完全采纳**。将 `device_id` 确立为“半机密凭据”不变量，确立 About 面板展示脱敏与日志脱敏红线规范。 | ✅ 红线已确立 |
| **S5** | 🟠 中 | 换盘或指纹不足 2 项会导致 `device_id` 自身改变，重造 403 死锁 | **完全采纳**。引入多维容错证据链（Device ID 相同 OR 主板 UUID 匹配 OR 有效 .lic），彻底化解换盘死锁。 | ✅ 机制已设计 |
| **S6** | 🟡 低 | §8.3 复述为“旧公钥云端解绑”，措辞不实 | **修正措辞**。已在 §8.3 修正为“旧公钥在云端被新公钥覆盖并失效”。 | ✅ 措辞已修正 |
