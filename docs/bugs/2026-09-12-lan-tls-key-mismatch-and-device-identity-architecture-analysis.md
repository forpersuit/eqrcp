...
6. **残余登记 2（`device_id` 授权强度与不变量定义）——【完全合理，推进文档落地】**：
   - 明确不变量：前端 About 面板对 `device_id` 实施的前 8 后 4 视觉掩码（`dev_9f82…3b9c`）定位为**纵深防御**（防肩窥、防截屏分享）；复制按钮保留完整明文是为了满足用户向客服提交排查凭证与多端授权的刚需。`device_id` 并非不可泄露的私钥，而是原主设备授权核验凭证；换绑防线的物理实质在于**服务端行绑定的原主凭据强一致比对 ∧ 客户端持有对应私钥**；云端频控仅降低未授权探测速率，不构成授权本身，且 per-node 桶键随客户端身份轮换而重置。
...
2. **R22 裁决（去神化承重墙，明确频控与授权边界）——【完全合理，推进文档落地】**：
   ...
   - **落地推进**：已在 §25.1.1 第 6 项中删去将频控称为“承重墙”的表述，清晰确立物理事实：`device_id` 为前端明文凭证（复制按钮全值导出以满足运维工单刚需），换绑防线的真正承重墙在于**服务端 D1 行绑定的强一致匹配 ∧ 客户端持有对应私钥**；频控仅作为节流降速辅助手段。
