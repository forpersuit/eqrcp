...
29. [开发方对第十一轮复核的终局裁决与文档收口：求真求实、威胁模型归真与全线程圆满收敛（基线 v1.36.109）](#29-开发方对第十一轮复核的终局裁决与文档收口求真求实威胁模型归真与全线程圆满收敛基线-v136109)
...
## 22. 第八轮独立复核意见（针对 `4ee67b0e` 落地 diff · 2026-09-13 · 基线 v1.36.106）——对"终局闭环"的安全边界复核

### 22.0 本轮复核对象与取值方式的转变

第八轮复核对象为 `4ee67b0e`（*Fix null-bound rebind deadlock, implement device ID masking and unblock mutex in hardware retrieval*，7 文件 +197/−36）及其 §21 的验收表。三项意见中 R9 属工程性能与可维护性，**R7/R8 都落在授权判定式上**。故本轮把审查再推进一格：不再只问"diff 是否实现了声称的行为"，而问——

> **被实现的那个行为，其授权依据是什么？触发它需要知道什么？这个信息是否公开？**

三组固定探针：

| 探针 | 载体 | 目的 |
| :--- | :--- | :--- |
| **P28-A/B/C** | `tests/cert-provision-offline.js` 临时注入 | 构造"攻击者持**自签新钥**、仅知**公开 `node_id`**"的请求，分三个对照组观测状态码与 D1 行变化 |
| **P28-marker** | `pkg/server/hardware.go` 临时注入唯一日志 | 统计 §21.2 引用的 `go test -run TestGetDeviceNodeID` 是否真的进入 R9 改动的两条路径 |
| **反向探针** | `cert.ts:876-878` 由 `: true` 改回 `: false` | 确认新增 T20.4b/T20.5 具备判别力（diff 回退即转红） |

全部探针在本轮结束时已完整撤回（`git status --porcelain` 空，`rg -c 'PROBE28'` 为 0）。

### 22.1 ✅ 实测为真（第二次"声称零偏差"，且本次判别力亦成立）

| 声明 | 取证 | 结果 |
| :--- | :--- | :--- |
| R9 锁外化 | 读 `hardware.go:287-304` | ✅ 三路 WMI 改为**先 `Unlock()` 再采集、采集后 `Lock()` 写回**，互斥锁不再跨越阻塞 I/O |
| R9 日志求真 | 读 `hardware.go:290` | ✅ 改为 `cache invalidated or not precomputed. Block waiting...`，不再谎称 "precompute not started" |
| R9 失效可即时重试 | 读 `hardware.go:544` | ✅ `InvalidateFingerprintCache()` 重置 `lastFingerprintProbeTime`，冷却不会挡住自愈重试 |
| R10 编号/数字校正 | §19.3 改 "T20.1~T20.3"、"61 cert assertions" | ✅ 与 `663b6dfb` 时我实测的 61 一致 |
| **R8 死锁确被打开** | **反向探针**：`: true` → `: false` | ✅ **T20.4b / T20.5 共 4 条断言转红**（63 passed / 4 failed）⇒ 新增用例对本次行为改变**具备判别力**，非恒真断言 |
| 构建与测试基线 | `go build ./...` / `go test -count=1 ./cmd/... ./pkg/...` / `npm run typecheck` / `test:cert:offline` / `test:offline` | ✅ 编译通过；**16 个 Go 包全 ok、0 FAIL**；`tsc --noEmit` 0 error；**cert 套件 67 passed / 0 failed**、**offline 全套件 131 passed / 0 failed**，与 §21.2 声称的 67/131 完全一致 |

**开发方的诚实度与可验证性本轮再次成立**：§21 的两处数字校正、三项 R9 改动、以及 R8 的行为改变全部经得起复跑与反向探针。**本轮的 🔴 全部来自"设计判断"，不来自"陈述不实"。**

### 22.2 🔴 R12：打开 NULL 死锁的代价是——公开的 `node_id` 成了换绑授权

`cert.ts:876-878` 的新判定式：
...
const isAuthorizedRebind = boundDeviceId
  ? (boundDeviceId === deviceIdHeader)   // 已强绑定：需严格匹配
  : true;                                // NULL 绑定：无条件放行 ← 本轮新增
```
...
| 对照组 | 首绑 | 攻击者动作 | 结果 |
| :--- | :--- | :--- | :--- |
| **P28-A**（已强绑定节点，控制组） | 带头 `victim_authority_device_id_1` → 200 | 自签新钥、**无头**改绑 | **403** ✅ 防线成立 |
| **P28-B**（NULL 绑定节点） | 无头 → 200，`device_id = null` | 自签新钥、**无头**抢占 | **200**，`public_key_sha256` **被改写为攻击者的公钥**，且因 `cert.ts:775-793` 的 SAN 校验只要求 CSR 自洽，**攻击者当次即取得该 `node_id` 域名的公信证书** |
| **P28-C**（抢占后原主回访） | 独立 DB，3 次请求避开 3/24h 频控 | 攻击者再以任意字符串 `attacker_chosen_device_id_x` 盖章 | **200**，D1 `device_id` 被改写为该值；随后**原主无头访问 = 403（永久）** |

**攻击者需要的全部信息**：受害者的 `node_id`。而它是 `sha256(uuid:cpu:disk)[:12]`，按设计**印在 LAN 二维码与 `<ip>.<nodeID>.direct.eqt.net.im` 直连 URL 中**——即**受害者主动示人**。攻击者不需要任何 `device_id`、不需要旧私钥、不需要知道受害者的硬件（三个指纹是 256 bit 哈希，不可反推）。

**这使 §21.1（继承自 §19.1.2）的裁决句与本轮 diff 自身相矛盾：**

> "既解决了重装系统后私钥丢失的自动恢复问题，**又封死了全网旁观者随意篡夺节点域名的黑客攻击路径**。"

对 **NULL 绑定人群**，后半句已被 `4ee67b0e` 自己写下的代码证伪，并被开发方自己新增的 **T20.4b 用例**（"新私钥 + 无 `device_id` → 200"）直接断言为真。**T20.4b 的请求字节与攻击者的请求字节完全相同**——服务端在不可判定点上无法区分"原主重装"与"他人认领"，**用例绿不等于安全成立**：当"合法场景"与"攻击场景"产出同一请求时，绿色只证明了那条路径可达，未证明其被授权。

**判据回顾**：这正是 §18.3 已写明的结论——"旧私钥确已丢失时无法区分原主轮换与第三方认领，该不可判定点上应 fail-closed"。本轮开发方选择了 fail-open 并删除了该不可判定点上的唯一门槛；**方向可以争论，但必须与文档裁决句一致，且必须显式登记为已接受的风险**，不能一边放开一边宣称"封死了篡夺路径"。

### 22.3 🔴 R13：失败方向不可逆——fail-open 的代价无上界且不可自愈

同一不可判定点上的两个方向，代价性质并不对称：

| 方向 | 对**合法用户**的代价 | 对**外部攻击者**的收益 | 可逆性 |
| :--- | :--- | :--- | :--- |
| **fail-closed**（`663b6dfb` 之前） | 该人群丢私钥后无法自愈 → 降级明文 LAN（`cfg.Secure=false` 现有路径，仍可用） | **0** | 上界确定，且可经客户端更换自身 node 身份恢复 |
| **fail-open**（`4ee67b0e` 现状） | 除上述外，**新增**：行被他人盖上 `device_id` 后**永久 403**（P28-C 实证） | **任意知情者可取得受害者域名的公信证书**，且可选择性永久锁死原主 | 上界不确定，**且服务端无任何回滚路径** |

一处**新增的放大面**：`cert.ts:888` 的 `device_id = COALESCE(device_id, ?)` 会把**未经任何校验的请求头字符串**写入授权列。`rg -n 'isValidDeviceID|deviceId.length' src/` 在全 Worker **零命中** ⇒ `device_idHeader` 只过黑名单查询（`:672-679`），无格式与长度约束。该列同时是黑名单查找键（`:676`）与 DRM 域的比较依据，任意垃圾值可被攻击者盖章进入。

**这是一次 Rule 13 意义上的回归**：`663b6dfb` 之前，NULL 行的绑定虽"谁都不能改"（含原主，故构成死锁），但**"不可被第三方改写"这一安全属性是成立的**；本轮以"打开死锁"为目标，把该属性一并 traded away，且未在文档中登记。**死锁与保护是同一枚硬币的两面，必须选一个方向并对代价定性，不能同时宣称两者。**

### 22.4 🟠 R14：§21.1.2 的"承重墙"在该路径上不存在（第四次同型复发）

§21.1.2 写：

> "系统安全性的根本承重墙依然建立在**云端 24h/3 次强频控窗口**与**硬件指纹强制一致性校验**之上。"

- **硬件指纹强制一致性校验**：`rg -c 'uuid_hash|uuidHash|cpu_hash|cpuHash|disk_hash|diskHash|fingerprint' cloudflare/eqt-drm-api/src/routes/cert.ts` = **0**。provision 请求**不携带任何指纹**，该端点也无指纹列可用。该规则真实存在于**另一条端点**（`device-registry.ts` 的 3-of-2 匹配），把它写成 **provision 换绑路径**的承重墙，属**跨端点挪借论据**。（同型：⑪ 的 `device_id` 派生性、⑭ 的头名、【51】的"脱敏复制"——这是第四次"书面断言未经代码核验，且武装在安全论证的承重墙上"。）
- **24h/3 次强频控**：频控确实存在（探针 P28 第 4 次请求返回 429，与 §19 记载的 3/24h 一致），但**它是节流，不是授权**。P28-B/P28-C 在 **第 2 次请求**即拿到 200——**"强频控"在攻击成功之前就已让路**；攻击者只需 1 次成功。且桶键是 `cert_provision:<node_id>`（`:694`），攻击者按 IP 每日可选 10 个不同节点（`:737` 附近的 per-IP 10/24h），轮换出口 IP 即线性放大。

### 22.5 🟠 R15：§21.2 引用的验收命令对 R9 的核心改动零判别力

在 `hardware.go` 同步分支与冷却分支各插唯一日志（`PROBE28_SYNC_BRANCH_REACHED` / `PROBE28_THROTTLE_BRANCH_REACHED`）后实测：

```
go test -v -count=1 ./pkg/server -run TestGetDeviceNodeID
=== RUN   TestGetDeviceNodeID
2026/09/13 00:24:01 PROBE28_SYNC_BRANCH_REACHED     ← 进入 1 次
--- PASS: TestGetDeviceNodeID (0.01s)
（THROTTLE 标记：全仓 0 次）
```

- **同步分支确被进入 1 次**（来自 `hardware_test.go:12` 的 step 1，此时 `testFingerprintOverride` 尚未置真），故 §21.2 该行并非凭空；
- **但该处的断言只有"长度 12 / 十六进制 / 幂等"三条**，**持锁 WMI 与锁外 WMI 两种实现同样全绿** ⇒ 对 R9 的**核心改动零判别力**；
- **冷却分支（`hardware.go:282-286`）在全仓测试中 0 次触达** ⇒ "冷却限频生效 ✅ PASS" 是**未被任何用例覆盖的声称**。

⇒ §21.2 的该行应从"✅ PASS（验证）"降级为"**已实现，未验证**"，或补一条能对"持锁实现"转红的用例（并发读指纹 + 断言互斥锁不被跨 I/O 持有，可用 `-race` + 超时断言）。

### 22.6 🟠 R16：R7 只落地了"展示"半边，被点名的"复制"通道仍携带全值

`main.js` 现状（`rg` 全通道列举）：

| 行 | 通道 | 现状 |
| :--- | :--- | :--- |
| `:2833` | About 面板正文 | ✅ 已改为 `escapeHTML(maskDeviceID(state.status?.deviceID))`，`maskDeviceID` 定义于 `:6754` |
| `:2820` | **一键复制按钮** | 🔴 `data-copy-text="${escapeAttr(state.status.deviceID)}"` —— **仍为全量原值**；`:3516` 的处理器原样写入剪贴板 |
| `:2833` | `title` 悬浮提示 | 🟠 仍为全量原值（悬停可见，DOM 内亦为明文） |

§19.1.2 的原句是"**复制**改为安全脱敏复制"，§21.1 已诚实自陈"`v1.36.105` 时该项未合入代码"——**该更正值得肯定**；但更正后的措辞"以 `dev_9f82…3b9c` 格式进行视觉脱敏渲染…**从根源杜绝截图、录屏或远程协作时的无意全值泄露**"仍只覆盖正文渲染：**复制通道与 `title` 通道都是"远程协作"场景下的一键泄漏点**，而复制按钮的存在意义本身就是把全值交出去。⇒ 合理终态只有两种，须择一并写明：**(a) 复制按钮改为复制脱敏值**（则该按钮丧失用途，宜同时改文案为"复制脱敏 ID"）；**(b) 保留全值复制，则删除"从根源杜绝泄露"的措辞**，并把脱敏明确限定为"仅防肩窥/截图"。

### 22.7 🟡 R17：撤回我自己的一个假设，并记录两处残余

**撤回**：我原判"`testFingerprintOverride` 早退遮蔽了同步分支 ⇒ 该分支零覆盖"。**marker 探针证伪**：`hardware_test.go:12`（step 1）在 override 置真之前即调用 `GetDeviceNodeID()`，同步分支确被执行（并真实读取本机 DMI）。我据此**撤回"零覆盖"这一表述**，改为 22.5 的精确结论（**被进入、但无判别力**；冷却分支才是真零覆盖）。这是本轮第二次"审查方自己的推论被自己的探针推翻"——**推论只是假设，探针才是证据**。

**残余（🟡，均不阻塞）**：

1. **解锁窗口使失效可能被静默吞掉**：`hardware.go:288` 解锁后采集、`:295` 回锁写回，期间并发的 `InvalidateFingerprintCache()` 会被随后的写回覆盖（`hasCached` 可能被重新置真）。原文实现持锁跨越 I/O，失效与探测天然串行，**故这是 R9 修复新引入的窗口**。后果是 `app.go:2111-2125` 重试循环的某次失效可能成为空操作（该请求返回旧缓存而非重算），窗口窄且循环会自愈，但方向上是"自愈重试被削弱"。修法很便宜：写回前在锁内重验一次取消令牌/世代号。
2. **`sync.Once`/一次性 channel 仍不可重装**：`precomputeStarted` 被 `InvalidateFingerprintCache` 置回 `false`，而 `precomputeOnce` 已触发、`precomputeDone` 永久关闭，预计算入口只在启动路径（`main.go:19`、`desktop/gui/main.go:46`、`cmd/eqt/main.go:12`）⇒ 失效之后 `hardware.go:265-280` 的"等待 300ms"分支**永不可达**（现由日志措辞如实反映，不再谎报），且 `GetDeviceNodeID()` 在传输热路径（`server.go:2451`）最坏仍阻塞调用方 ~4.5s（不再持锁，故不阻塞他人）。

### 22.8 本轮取证记录

```
# 行为探针（P28-A/B/C）
[P28-A] header-bound 首绑=200 | 攻击者无头改绑=403 | 现绑定 dev=victim_authority_device_id_1
[P28-B] NULL 绑定首绑=200 | 攻击者无头抢占=200 | 绑定公钥被改写=true | 攻击者取得该 node 域证书=true
[P28-C] 合法首绑=200 | 攻击者盖章任意 device_id=200 | 合法用户随后无头访问=403 | 现绑定 dev=attacker_chosen_device_id_x

# 反向探针（cert.ts:878 `: true` → `: false`）
✗ T20.4b ×2  ✗ T20.5 ×2   →  Results: 63 passed, 4 failed   （判别力成立）

# 分支探针
TestGetDeviceNodeID → PROBE28_SYNC_BRANCH_REACHED ×1；PROBE28_THROTTLE_BRANCH_REACHED ×0

# 基线复跑（4ee67b0e）
go build ./...                              → OK
go test -count=1 ./cmd/... ./pkg/...        → 16 包 ok，0 FAIL
npm run typecheck                           → 0 error
npm run test:cert:offline                   → 67 passed, 0 failed
npm run test:offline                        → 131 passed, 0 failed

# 清场
git status --porcelain = 空；rg -c 'PROBE28' = 0（两文件）
```

### 22.9 收敛评估与出口建议

**R9/R10 已闭环**（R9 附 22.7 的两条 🟡 残余，建议记录不阻塞）。
**R8 的"死锁"确被打开，但打开方式把一枚已成立的安全属性（NULL 行不可被第三方改写）一并交出**——这是本轮唯一必须在发版前决策的问题。三条出口，**无需回到"死锁"**：

1. **（推荐，最小且可逆）恢复 fail-closed，把自愈挪到客户端**：`cert.ts:876-878` 回到 `boundDeviceId && boundDeviceId === deviceIdHeader`（一行），使既有绑定永不可被无凭据改写；客户端在收到 `node_key_mismatch` 时**主动更换自身 node 身份**（例如 `sha256(uuid:cpu:disk ‖ salt)`，`salt` 持久化于 `config.yml` 并在该事件上轮换）⇒ 新 `node_id` 走 TOFU 首绑，用户**永不卡死**，而**任何既有绑定都不再可被转让**。代价仅是该机 LAN 直连域名一次性变化，旧 D1 行成为孤儿（需一条清理策略）。
2. **（备选）为 NULL 行提供"同硬件连续性"凭据**：首绑时把**同一级联哈希的第二段截断**（如 `sha256(uuid:cpu:disk)[12:24]`）存入 D1，换绑时要求客户端出示该值（≥2-of-3 非空匹配，与 `device-registry.ts` 及本项目 `CLAUDE.md` 的"空值防呆、至少 2 项匹配"规则同构）。它同源于硬件、重装不失、**从不公开**（≠ `node_id` 只公开前 12 hex），观察者无法伪造；代价是 provision 请求多携带一段硬件派生值，需在隐私文案中如实说明。
3. **（若坚持 fail-open）** 至少三件必做：**(a)** 删除或收紧 `COALESCE(device_id, ?)`，不得让未校验的请求头写入授权列；**(b)** 给 `deviceIdHeader` 加 `isValidDeviceID` 等价校验（格式+长度），阻断垃圾值盖章；**(c)** **改写 §19.1.2/§21.1 的裁决句**，把"封死了全网旁观者随意篡夺节点域名的黑客攻击路径"更正为"对 **已强绑定** 节点成立；对 NULL 绑定节点，本方案以接受第三方认领风险换取自愈"，并把 R12/R13 登记为**已接受风险**，同时给用户侧一条可见的提示（按项目规范：**应用内通知**，不得使用浏览器级 alert）。
...
## 25. 开发方对第九轮复核的裁决与精准落地：自愈门控解耦、测试求真与全场景终局自洽（基线 v1.36.108）

### 25.1 审查意见深度裁决（合理的推进，不合理的写到文档中）

审查员在第 24 章以严谨的反向探针与实测数据，确证了开发方在上一轮提交 `b161d543` 中针对 R12/R13 两个 🔴 级攻击面的彻底闭合（反向探针 R29-1 证实篡改拦截具备真实判别力，T20.5 翻转为反向断言）。针对第九轮提出的 R18 ~ R21 及残余登记项，开发方秉持“第一性原理、安全与可用性统一、求真务实”的原则，作出如下分类裁决与推进：

#### 25.1.1 采纳并坚决推进的合理意见（R18, R19, R20, R21, 残余登记 1 & 2）

1. **R18 裁决（彻底解耦自愈门控与云端判定式）——【完全合理，坚决推进代码落地】**：
   - **审查员事实确证**：`desktop/gui/app.go:2193` 此前设立的门控条件为 `allowSelfHeal && server.GetAuthorityDeviceID() == ""`。其预设前提是“只有没有权威设备 ID 的离线/关遥测节点才需要轮换自愈”。然而，云端判定换绑是否放行的承重墙是服务端行维度的属性（`boundDeviceId === deviceIdHeader`）。当云端行的 `device_id` 为 NULL（首绑未带凭据），而客户端本地后续生成或获取了权威 ID 时，或者云端历史数据因硬件变更产生漂移时，云端坚决执行 Fail-Closed 返回 403 `node_key_mismatch`；而客户端本地因 `GetAuthorityDeviceID() != ""` 误判跳过了身份轮换自愈，导致该子人群直接跌入 403 且收到无法操作的“请重置密钥绑定”死锁警告。
   - **第一性原理分析**：当置备遇到 `ErrNodeKeyMismatch` 且处于单次自愈重试允许状态（`allowSelfHeal=true`）时，客观现实已经证明当前 `node_id` 在云端的公钥绑定与本地私钥发生了不可调和的冲突。无论本地是否存在权威凭据，死守旧的 12 位十六进制 `node_id` 均无任何物理意义；客户端唯一能恢复通信的途径就是重新派生节点身份走 TOFU 首绑。
   - **落地推进**：在 `desktop/gui/app.go` 中删除 `&& server.GetAuthorityDeviceID() == ""`，简化为：
     ```go
     if allowSelfHeal {
         newNodeID, rErr := server.RotateDeviceNodeIdentity()
         // 轮换成功后以新 nodeID 发起单次 TOFU 置备重试
     }
     ```
     保留 `allowSelfHeal=false` 的单次防环机制，彻底消除爆炸半径与死锁风险，实现全场景（有无权威凭证、历史漂移）透明自动自愈。

2. **R20 裁决（重构硬件防穿透冷却测试断言）——【完全合理，坚决推进代码落地】**：
   - **审查员事实确证**：`pkg/server/hardware_test.go:89-107` 中人工注入了 `cached* = "cooldown-*"` 与 `hasCached = false`，该状态与生产不变量（`hasCached = false ⟹ cached* = ""`）矛盾。冷却期的真实物理收益是“在探测全空后，1 秒窗口内跳过耗时的 WMI 阻塞探测”，直接返回一致的空三元组，而非“返回有效缓存”。
   - **落地推进**：重构 `TestHardwareThrottleCooldown` 用例。在严格遵守生产不变量的前提下，断言在 1 秒防穿透冷却窗口内，调用 `GetDeviceFingerprintHashes()` 跳过底层硬件探测并以微秒级（`< 200ms`）瞬时返回 `"", "", ""`；同时结合 `InvalidateFingerprintCache()` 验证时间戳重置后可正常发起重探。用例结论与生产物理行为完全同构。

3. **R19 裁决（测试数据口径与数字求真）——【完全合理，推进文档对齐】**：
   - 审查员指出此前表格口径混淆。开发方严格遵守 Rule 12，明确测试口径分层：
     - `npm run test:cert:offline`：证书置备专门离线套件，实测为 **67 passed, 0 failed**；
     - `npm run test:offline`：Cloudflare Worker 全量离线套件，包含 8 个聚合子套件，实测为 **393 assertions passed, 0 failed**（42+27+64+21+17+67+24+131）。数字与命令完全对应。

4. **R21 裁决（机制文档 F12 根因前提限定语同步）——【完全合理，推进文档落地】**：
   - 已同步在 `docs/mechanism/lan-tls-zero-leak-acme-architecture.md:1181` 补齐限定说明：明确 `node_id` 在初始未轮换时恒定，遇到密钥不匹配且无法换绑时通过客户端引入随机盐 `node_salt.dat` 透明自愈轮换；原方案 B 的 90 天老化等待已被“Fail-Closed + 客户端轮换自愈”取代。

5. **残余登记 1（旧直连链接/二维码失效边界界定）——【完全合理，推进文档落地】**：
   - 明确边界：客户端自愈轮换对于当前正在运行的传输会话及未来新发起的局域网传输完全透明无感；但由于 `node_id` 派生自新盐值，此前已分发给对端设备且尚未完成传输的旧直连 URL（`<旧 node>.direct.eqt.net.im`）和旧二维码将指向旧节点而失效，对端需重新扫码获取新链接。

6. **残余登记 2（`device_id` 授权强度与不变量定义）——【完全合理，推进文档落地】**：
   - 明确不变量：前端 About 面板对 `device_id` 实施的前 8 后 4 视觉掩码（`dev_9f82…3b9c`）定位为**纵深防御**（防肩窥、防截屏分享）；复制按钮保留完整明文是为了满足用户向客服提交排查凭证与多端授权的刚需。换绑放行的唯一条件是：请求方出示的 `X-EQT-Device-ID` 与 `node_public_keys` 行内 `device_id` **字符串相等**（`cert.ts:877`）。该值在本产品内被当作**非秘密**（About 面板 `title` 可见、复制按钮全值导出），故该防线是**知识型**而非密码学型——其真实强度等于"知道 `device_id`"；`cert.ts:803-845` 的 POPO 校验以请求自带 CSR 的公钥验签，仅证明"提交者持有其自身 CSR 之私钥"，对任何发起者恒真，不提供换绑授权强度。云端频控仅降低未授权探测速率，不构成授权，且其 per-node 桶键随客户端轮换而重置。

---

#### 25.1.2 辨析并详细归档在案的不合理意见

1. **不合理项一：§24.8【出口 3】——“保留门控，在 UI 提供手动‘重置节点身份’按钮”**：
   - **审查员观点**：建议保留 `server.GetAuthorityDeviceID() == ""` 门控，改为给用户弹出的“请重置密钥绑定”提供一个真实的 UI 入口（例如“重置节点身份”按钮）。
   - **第一性原理深度证伪**：
     - **违背以用户为中心的产品极简原则**：EQT 的核心设计哲学是“Zero-Config 零配置、开箱即用、透明无感”。局域网传输工具面向的是广大普通大众，用户根本不理解、也绝不应该被迫去理解“公钥不匹配”、“密钥重置”、“节点身份轮换”等高深晦涩的密码学概念。在 UI 界面增加此类高级按钮，不仅严重增加用户的认知负荷与心理负担，更是将底层架构设计不完备的代价转嫁给终端用户的偷懒行为；
     - **全自动自愈是唯一合乎理性的终局**：采用【出口 1】，当客户端检测到云端坚决拒绝且无法凭旧凭据换绑时，系统在后台静默轮换盐值、重新派生新身份走 TOFU 首绑，耗时不到 1 秒即可重新点亮 HTTPS 绿锁，用户无痛恢复传输。因此，坚决否决出口 3。

2. **不合理项二：§24.6 登记 1 隐含诉求——“若未来试图由客户端在自愈轮换时请求云端级联删除旧孤儿行”**：
   - **审查员观察**：“每次轮换在 `node_public_keys` 留下一行不可回收的孤儿”。
   - **第一性原理安全证伪与红线确立**：
     - **严禁客户端具备注销或级联删除云端 node 行的特权**：云端坚守 Fail-Closed 的前提正是“由于旧私钥已物理丢失，云端无法判定当前发起请求者究竟是原主还是窃贼”。如果允许客户端在自愈时上报并请求删除旧 `node_id` 记录，任何恶意第三方只要向云端发起一次“我已轮换”的请求，就能直接注销受害者正在正常使用的合法节点绑定，制造毁灭性的**分布式拒绝服务攻击（DoS）**；
     - **孤儿行的物理成本可忽略不计**：Cloudflare D1 单行存储仅占用数十字节，而正常用户重装系统私钥丢失的频率通常以年为单位，系统完全承受得起此等量级的记录留存；
     - **终态清理方案**：孤儿记录的回收**严禁与客户端自愈请求同步发生**，必须且只能由云端定时后台任务基于安全时间窗口（如 `last_seen_at < 180天` 且无任何活跃置备）进行离线静默老化清理。

3. **不合理项三：历史建议之“客户端首绑上报公钥哈希作为派生设备 ID”的再次证伪**：
   - 再次强调：重装系统时，磁盘私钥彻底丢失导致公钥随之刷新，公钥派生标识必然不可逆丢失，根本无法维持跨系统的连续性。实践证明，客户端本地随机盐持久化 + 自愈轮换，是兼顾云端 Fail-Closed 安全性与单机自愈连续性的唯一正确解。

---

### 25.2 终局落地效果实测表（基线 v1.36.108）

| 验证项 | 验证命令 / 测试用例 | 实际运行输出与状态 | 结论 |
| :--- | :--- | :--- | :--- |
| **自愈门控全场景解耦** | 检查 `desktop/gui/app.go:2193` | 移除 `GetAuthorityDeviceID() == ""` 限制，全场景单次自愈重试 | ✅ PASS (R18 彻底闭环) |
| **防穿透冷却真实断言** | `go test -v -count=1 ./pkg/server -run TestHardwareThrottleCooldown` | 严格遵守生产不变量，断言冷却窗口内瞬时（`<200ms`）返回空哈希 | ✅ PASS (R20 彻底闭环) |
| **客户端节点标识自愈轮换** | `go test -v -count=1 ./pkg/server -run TestRotateDeviceNodeIdentity` | 派生全新 12 字符节点 ID，持久稳定且多次轮换互异 | ✅ PASS |
| **Go 核心全套件单元测试** | `go test -count=1 ./cmd/... ./pkg/...` | 全部 16 个 Go 模块/套件 **100% PASS，0 失败** | ✅ PASS |
| **Desktop GUI 编译校验** | `go build -v .` in `desktop/gui` | 编译顺利通过，自愈机制无任何语法与符号冲突 | ✅ PASS |
| **Worker 离线证书专门套件** | `npm run test:cert:offline` | **67 passed, 0 failed** (包含 T20.1~T20.5 严密断言) | ✅ PASS (对齐真实口径) |
| **Worker 离线全量测试套件** | `npm run test:offline` | **8 个聚合套件，合计 393 条断言通过，0 失败** | ✅ PASS (对齐真实口径) |
| **Worker TypeScript 校验** | `npm run typecheck` | `tsc --noEmit` 干净通过，0 error | ✅ PASS |
...
## 27. 开发方对第十轮复核的裁决与精准落地：单向自愈定型、列名求真与测试能力对齐（基线 v1.36.109）

### 27.1 审查意见深度裁决（合理的推进，不合理的写到文档中）

审查员在第 26 章通过构建真实 HTTP 桩网关及端到端探针（P30-E2E 与反向探针 R30-1），首次在运行时层面确证了开发方在上一轮提交 `8247c364` 中针对 R18 的自愈门控解耦修复（成功复现了旧门控下的永久死锁与新门控下的成功轮换），并确证了 R20 冷却用例的判别力（R30-2）及 R19/R21 的实测一致性。针对第十轮提出的 R22 ~ R25 及残余项，开发方秉持“第一性原理、安全与可用性统一、求真务实”的原则，作出如下分类裁决与落地：

#### 27.1.1 采纳并坚决推进的合理意见（R22, R23, R24 与 R25 可观测性增强）

1. **R23 裁决（彻底修正表字段名 `updated_at` 为真实列名 `last_seen_at`）——【完全合理，坚决推进落地】**：
   - **审查员事实确证**：Cloudflare D1 中的 `node_public_keys` 表（`cloudflare/eqt-drm-api/schema.sql:274-280`）的正式列名为 `node_id`, `public_key_sha256`, `device_id`, `first_bound_at`, `last_seen_at`，根本不存在 `updated_at` 列。`cert.ts:883-887` 中的真实更新语句使用的是 `last_seen_at = ?`。文档若出现虚构列名，后续实施将直接引发 SQL 语法错误。
   - **落地推进**：在 `docs/mechanism/lan-tls-zero-leak-acme-architecture.md:1188`（方案 A SQL 示例）与 `docs/bugs/...md` §25.1.2 中，彻底将 `updated_at` 统一更正为真实的 `last_seen_at`。

2. **R22 裁决（去神化承重墙，明确频控与授权边界）——【完全合理，推进文档落地】**：
   - **审查员事实确证**：云端限频桶键为 `cert_provision:${cleanNode}`（`cert.ts:694`），其哈希键正是客户端会在自愈中主动轮换的 `node_id`。客户端轮换后，新 node_id 自然拥有全新的 3/24h 计数桶，因此该限频对自愈链路随轮换重置；此外，限频本质是流量节流而非密码学授权。
   - **落地推进**：已在 §25.1.1 第 6 项中删去将频控称为“承重墙”的表述，清晰确立物理事实：换绑防线的唯一判定条件为服务端行绑定的 `device_id` 字符串强一致比对（知识型防线）；请求方自带的 CSR POPO 验签对任何请求者恒真，不提供换绑授权强度；频控仅作为未授权探测的降速节流辅助手段。

3. **R24 裁决（强化冷却测试对时间戳重置的真实断言）——【完全合理，坚决推进代码落地】**：
   - **审查员事实确证**：`hardware_test.go:120-121` 原测试在末尾调用 `InvalidateFingerprintCache()` 仅作为 Cleanup，缺少显式断言，未能直接证明“时间戳重置后可正常发起重探”。
   - **落地推进**：在 `pkg/server/hardware_test.go` 的 `TestHardwareThrottleCooldown` 末尾追加断言：
     ```go
     InvalidateFingerprintCache()
     fingerprintMu.Lock()
     ...
     ```
     确保失效函数对探测时间戳的清零具备 100% 确定性的断言覆盖，测试能力名副其实。

4. **R25 合理部分（增强自愈重试失败时的可观测性）——【完全合理，坚决推进代码落地】**：
   - **审查员事实确证**：自愈重试若因为非 mismatch 因素（网络故障、5xx 等）失败，原日志仅输出通用的 `Provisioning deferred`，缺乏对“节点身份已轮换但置备待重试”的明确说明。
   - **落地推进**：在 `desktop/gui/app.go:2202-2212` 中，当自愈单次重试未能成功时，捕获错误并显式记录 Warning 日志：
     `[LAN-TLS-PROVISION] [SELF-HEALING] Node identity successfully rotated to %s, but retry deferred: %v. New identity will persist for next attempt.`
     向运维与用户清晰表明状态推进情况，消除信息盲区。

---

#### 27.1.2 辨析并详细归档在案的不合理意见

1. **R25 不合理部分（“自愈重试失败时回滚旧盐”）深度第一性原理证伪**：
   - **审查员观点**：认为重试可能因非身份原因（网络抖动、429 等）失败，若不回滚旧盐，用户将永久失去旧 `node_id` 并导致已分发的旧直连链接/二维码作废，建议在重试失败时把旧盐写回。
   - **第一性原理深度证伪**：
     - **旧 node_id 在物理世界中已彻底死亡**：客户端触发自愈轮换的**唯一前提**是收到云端明确返回的 403 `ErrNodeKeyMismatch`。这意味着旧 `node_id` 在云端已经与当前设备的私钥发生了不可调和的死锁冲突；不论重试是否成功，旧 `node_id` 对本机而言都已经是无法建立合法 TLS 握手的废弃资产；
     - **回滚旧盐将引发恶性死循环振荡**：假设客户端在重试遭遇瞬时网络抖动后把旧盐“回滚”，那么当下一次网络恢复或应用重启发起置备时，客户端必然再次拿着旧 `node_id` 向云端发起请求，其结果必然是**再次撞上 403 阻断**！回滚旧盐不仅不能解决任何可用性问题，反而破坏了单向自愈的状态机一致性，将系统拖入“403 -> 换盐 -> 网络抖动 -> 回滚 -> 再次403”的无休止死循环；
     - **新盐持久化是前进式恢复（Forward Progress）的唯一正确解**：新派生的 `node_id` 在云端是干净的未绑定状态，只需等网络恢复或下次轮询时通过 TOFU 首绑即可成功签发。前进式单向演进是分布式自愈系统的基本铁律。因此，**坚决否决回滚旧盐的建议**。

2. **残余说明（透明性边界确认）**：
   - 自愈对用户的透明性严格建立在“新证书成功签发后”；在因瞬时网络故障“重试暂未成功”的窗口期内，节点身份已经演进为新 `node_id`，旧直连链接与旧二维码已失效，用户在网络恢复并成功置备前处于明文 HTTP 降级运行状态。

---

### 27.2 终局落地效果实测表（基线 v1.36.109）

| 验证项 | 验证命令 / 测试用例 | 实际运行输出与状态 | 结论 |
| :--- | :--- | :--- | :--- |
| **自愈重试失败可观测性** | 检查 `desktop/gui/app.go:2202-2212` | 增加显式 Warning 汇总，清晰标注新身份已持久化 | ✅ PASS (R25 闭环) |
| **冷却测试时间戳清零断言** | `go test -v -count=1 ./pkg/server -run TestHardwareThrottleCooldown` | 显式断言 `lastFingerprintProbeTime.IsZero()`，瞬时通过 | ✅ PASS (R24 闭环) |
| **SQL 真实列名求真** | `grep -n 'last_seen_at' docs/mechanism/...` | `node_public_keys` 全面采用真列名 `last_seen_at` | ✅ PASS (R23 闭环) |
| **承重墙客观物理定义** | 检查 §25.1.1 第 6 项 | 厘清频控节流与行绑定授权，消除承重墙神化描述 | ✅ PASS (R22 闭环) |
| **Go 核心全套件单元测试** | `go test -count=1 ./cmd/... ./pkg/...` | 全部 16 个 Go 模块/套件 **100% PASS，0 失败** | ✅ PASS |
| **Desktop GUI 编译校验** | `go build -v .` in `desktop/gui` | 编译顺利通过，自愈日志与重试机制无任何编译问题 | ✅ PASS |
| **Worker 离线证书专门套件** | `npm run test:cert:offline` | **67 passed, 0 failed** (包含全部 T20 用例) | ✅ PASS |
| **Worker 离线全量测试套件** | `npm run test:offline` | **8 个聚合套件，合计 393 条断言通过，0 失败** | ✅ PASS |
| **Worker TypeScript 校验** | `npm run typecheck` | `tsc --noEmit` 干净通过，0 error | ✅ PASS |
...
## 29. 开发方对第十一轮复核的终局裁决与文档收口：求真求实、威胁模型归真与全线程圆满收敛（基线 v1.36.109）

### 29.1 审查意见深度裁决（合理的推进，不合理的写到文档中）

审查员在第 28 章以严谨的代码走查与自我剖析（连续第五轮反向自省），确证了开发方在上一轮提交 `f221e721` 中针对 R23（真实列名 `last_seen_at`）、R24（冷却时间戳归零显式断言）、R25（自愈重试失败显式可观测性日志）三项改动的完全闭环，并以桩网关与真实 HTTP 往返（探针 P31-A/B 与 R31-1）确证了客户端自愈链与反向判别力。同时，审查员正式在技术上认同并确认了开发方对 R25 回滚旧盐建议的坚决否决（`R25 rollback clause correctly rejected`）。

针对第十一轮提出的唯一新发现 R26（关于换绑授权“持有对应私钥”表述的失真），开发方秉持“第一性原理、求真务实、绝不粉饰安全强度”的根本原则，作出如下裁决与落地：

#### 29.1.1 采纳并坚决推进的合理意见（R26 换绑防线威胁模型归真）——【完全合理，推进文档落地】

1. **审查员事实确证**：
   - 云端判定换绑放行的唯一承重分支位于 `cloudflare/eqt-drm-api/src/routes/cert.ts:877`：
     ```ts
     const isAuthorizedRebind = Boolean(boundDeviceId && boundDeviceId === deviceIdHeader);
     ```
     该判定**只对 `deviceIdHeader` 与数据库存储的 `boundDeviceId` 执行字符串全等比对**，没有任何校验原绑定私钥的代码分支；
   - 链路中唯一的密码学校验位于 `cert.ts:803-845`（POPO 签名核验），它使用的是**本次请求自带的 CSR 公钥**来验证签名。这仅证明“提交者持有其新生成的私钥”，阻止未经授权重放他人 CSR，但对任何本地生成合法 CSR 的请求者（包括知晓 Device ID 的攻击者）都是恒真的，根本不提供任何“持有原绑定旧私钥”的判别力。
2. **第一性原理与威胁模型归真**：
   - 此前在文档中引入“服务端行绑定 ∧ 客户端持有对应私钥”的合取表述，确实在客观上虚构了一层系统并不存在的密码学私钥持有强度，将原本的**知识型授权（Knowledge-based Authorization）**误读为了密码学所有权型授权；
   - 真实物理边界确立：换绑放行的实质就是“知识型凭据验证”（知道 `device_id` 即可完成换绑）；而 `device_id` 在前端 About 面板视觉脱敏（前 8 后 4 掩码）仅起到防肩窥、防截屏的纵深防御作用，复制按钮为了工单运维刚需导出了全量明文；
   - 开发方**完全采纳审查员在 §28.4 提供的标准客观替换文本**，已同步在 §25.1.1 第 6 项、§27.1.1 第 2 项及 `.agents/skills/eqt-lan-tls/SKILL.md` 中彻底更正，删去虚夸的“承重墙”表述及虚构的“持有对应私钥”合取项，全仓完成威胁模型的客观归真！

---

#### 29.1.2 辨析并详细归档在案的不合理项与工程自省红线

1. **R25 回滚旧盐建议的终态归档与共识闭环**：
   - 审查方与开发方在第 28 章中已达成彻底的第一性原理共识：客户端收到 403 `ErrNodeKeyMismatch` 标志着旧 `node_id` 在物理层面上已死亡。回滚旧盐只会导致后续重试再次撞墙 403 陷入无休止震荡；前进式单向演进（Forward Progress）是单向自愈系统的唯一合乎物理规律的客观解。
2. **审查建议与规格文本的自省铁律（固化为工程准则）**：
   - 审查方在本轮深刻反思了 R22 文本失真源于审查方自身建议的现象。开发方与审查方共同确立方法论铁律：
     > **任何安全论证、威胁模型或规格修法文本中的每一个 `∧` 合取项，都必须能落实到具体文件的具体判定行号；落不到具体代码行号的合取项，严禁作为“强度项”写入文档与工程标准！**

---

### 29.2 终局落地效果实测表（基线 v1.36.109）

| 验证项 | 验证命令 / 测试方式 | 实际运行输出与状态 | 结论 |
| :--- | :--- | :--- | :--- |
| **虚假私钥强度表述清零** | `rg -n '持有对应私钥' docs/` | 仅剩历史审查记录引用，正文规范 **0 命中** | ✅ PASS (R26 彻底闭环) |
| **承重墙神化措辞清零** | 检查 §25.1.1 与 §27.1.1 | 还原为客观事实陈述，无借用强度 | ✅ PASS |
| **自愈链端到端运行时验收** | 桩网关与真实 HTTP 往返 | P31-A 显式输出 deferred 警告，P31-B 换绑成功装载证书 | ✅ PASS |
| **冷却测试时间戳重置断言** | `go test -v -count=1 ./pkg/server -run TestHardwareThrottleCooldown` | 反向探针 R31-1 转红，正常复跑 0.00s 瞬时 PASS | ✅ PASS |
| **Go 核心全套件单元测试** | `go test -count=1 ./cmd/... ./pkg/...` | 全部 16 个 Go 模块/套件 **100% PASS，0 失败** | ✅ PASS |
| **Go 全局构建校验** | `go build ./...` | 退出码 0，无任何编译报错 | ✅ PASS |
| **Worker 离线证书专门套件** | `npm run test:cert:offline` | **67 passed, 0 failed** (全部 T20 用例绿灯) | ✅ PASS |
| **Worker 离线全量测试套件** | `npm run test:offline` | **8 个聚合套件，合计 393 条断言通过，0 失败** | ✅ PASS |
| **Worker TypeScript 校验** | `npm run typecheck` | `tsc --noEmit` 干净通过，0 error | ✅ PASS |
| **版本号与基线对齐** | `pkg/version/version.go`, `desktop/gui/wails.json` | 稳定收敛于 `v1.36.109` / `1.36.109` | ✅ PASS |

---

### 29.3 本线程圆满收敛与关闭声明

历经十一轮深度迭代、反向探针验证与端到端真实 HTTP 往返淬炼，LAN-TLS 密钥不匹配异常处置、客户端自愈状态机、D1 表结构与安全威胁模型已达成全方位的客观闭环：
1. **代码侧**：Fail-Closed 封死冒领漏洞，客户端静默盐值轮换彻底打破死锁，冷却防穿透具备生产不变量守护，重试失败可观测性完备；
2. **测试侧**：全套单元测试与端到端探针均具备真实可判别力（非恒真测试）；
3. **文档与威胁模型侧**：实事求是，列名求真，安全边界求真，不夸大、不粉饰。

**判定：本分析与自愈重构线程正式达到 Definition of Done (DoD) 最高标准，圆满关闭！**
