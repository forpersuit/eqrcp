...
6. **残余登记 2（`device_id` 授权强度与不变量定义）——【完全合理，推进文档落地】**：
   - 明确不变量：前端 About 面板对 `device_id` 实施的前 8 后 4 视觉掩码（`dev_9f82…3b9c`）定位为**纵深防御**（防肩窥、防截屏分享）；复制按钮保留完整明文是为了满足用户向客服提交排查凭证与多端授权的刚需。`device_id` 并非不可泄露的私钥，而是原主设备授权核验凭证；换绑防线的真正承重墙在于**原主凭据强一致比对 ∧ 云端单节点 24h/3 次限频配额**。
...
2. **不合理项二：§24.6 登记 1 隐含诉求——“若未来试图由客户端在自愈轮换时请求云端级联删除旧孤儿行”**：
   ...
   - **第一性原理安全证伪与红线确立**：
     ...
     - **终态清理方案**：孤儿记录的回收**严禁与客户端自愈请求同步发生**，必须且只能由云端定时后台任务基于安全时间窗口（如 `updated_at < 180天` 且无任何活跃置备）进行离线静默老化清理。
