...
|---|---|---|---|---|---|---|
| 1 | D1 备份策略（GitHub Actions cron + 恢复演练） | ⬜ 待开始 | — | — | — | 备份文件可下载恢复，季度演练通过 |
| 2 | 结构化日志中间件（所有 Worker） | ⬜ 待开始 | — | — | — | 每个请求输出 JSON 日志，Logpush 可查 |
| 3 | Logpush 配置（R2 目标） | ⬜ 待开始 | — | — | — | R2 中存在日志文件 |
| 4 | SLO 定义文档化 | ⬜ 待开始 | — | — | — | 本文档 §6.3 已定义，团队确认 |
| 5 | 健康检查端点增强（D1/R2 深度检测） | ⬜ 待开始 | — | — | — | 返回真实 D1 延迟，UptimeRobot 绿色 |
...
| # | 事项 | 状态 | 负责人 | 预计完成 | 实际完成 | 验证方式 |
|---|---|---|---|---|---|---|
| 7 | 请求级 trace_id（Worker + D1 + 桌面端） | ⬜ 待开始 | — | — | — | 响应含 X-Trace-Id，D1 可查 |
| 8 | 业务指标仪表盘（admin 页面） | ⬜ 待开始 | — | — | — | 5 个指标可查看，与 D1 查询一致 |
| 9 | 告警升级机制（Telegram + 邮件分层） | ⬜ 待开始 | — | — | — | CRITICAL 触发 1 分钟内 Telegram 通知 |

### P2 — 优化项，backlog

| # | 事项 | 状态 | 负责人 | 预计完成 | 实际完成 | 验证方式 |
|---|---|---|---|---|---|---|
| 10 | 部署流水线（GitHub Actions） | ⬜ 待开始 | — | — | — | push 到 main 自动部署 staging |
| 11 | 回滚策略（wrangler versions） | ⬜ 待开始 | — | — | — | 可秒级回滚到上一版本 |
| 12 | 成本监控（CF 预算告警） | ⬜ 待开始 | — | — | — | 仪表盘有预算告警配置 |
| 13 | 滥用检测自动化 | ⬜ 待开始 | — | — | — | 异常激活自动触发限流 |
