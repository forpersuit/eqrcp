...
启动 → 检测到 crash.dump
  ...
  └─ 方案 C：自动静默上报 + 小蓝点提示
      ...
      上报成功后，小蓝点提示 "上次崩溃已自动上报，点击查看详情"
      用户可打开反馈面板补充说明
```

**推荐方案 A** 的理由：
- 避免突兀弹窗打断用户操作（当前 crash-report 模态框是强中断）
- 小蓝点是渐进式提示，用户可忽略，也可跟进
- 复用现有反馈面板，无需新写 UI 组件
- 崩溃信息自动预填，降低用户反馈门槛
- 关闭即清理，不留残留
...
- `main.js`：`eqt:crash-report-pending` 事件改为设置标志位 + 小蓝点渲染，不再直接弹窗
...
| 8a | 崩溃恢复 UX 优化（小蓝点渐进提示 + 反馈面板预填） | ✅ 已完成 | — | — | 2026-08-05 | 小蓝点引导 → 反馈面板预填崩溃信息 → 关闭即清理；修复启动事件竞态（前端初始化主动 CheckCrashReport） |
...
| 2026-08-05 | 崩溃恢复 UX 设计评审：方案 A（渐进式小蓝点 + 反馈面板预填）确定为推荐方案，待排期开发 | 设计评审 |
