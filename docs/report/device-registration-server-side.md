...
> **同码多设备去重**：城市对按**无序对**去重（`min->max` 字典序规范化 key）。同一激活码在同一城市有 ≥2 台设备、另一城市 1 台时，只生成一条弧（而非方向各一条的反向冗余）；由于规范化后的端点决定方向，弧线方向在多次刷新间**确定**，不随行内顺序漂移。
...
> **✅ 实现状态（2026-08-03 完成并经过全面审查复审）**：本节待生效全套机制及服务端双侧预检已完全实现，通过代码复审与集成测试闭环。支持年付与终身全额买断独立隔离、14 天退款期基于 `last_purchased_at` 严格拦截、`auto_renew` 停用防双重扣款、客户端 `/verify` 与 `/activate` 惰性翻转生效、待生效及已生效退款/Adjustment 独立吊销归位，以及 Portal 升级预检接口 `POST /api/v1/user/upgrade-checkout` 与真实 `PRICE_LIFETIME_ID` 挂载。
...
> 更新：2026-08-05。M1、M2、M3 已全部实现。以下为当前状态与 M4 后续安排。

| 里程碑 | 内容 |
| --- | --- |
| M1 | ✅ **已完成**：device_registry 表 + register 端点 + activate 内联注册 + 启动序列改造 + verify 契约修正 + 遥测开关 |
| M2 | ✅ **已完成**：device_id 进签名载荷 + 双版本验签 + 手动续期 + 年付→终身升级 |
| M3 | ✅ **已完成**：/admin/devices/live + 大屏双色活跃口径 + 抛物线开关 |
| M4 | **见下方 §7.1**：原 M4 范围经重新评估后拆分，仅保留有效项 |

### 7.1 M4 重新评估与开发安排

> 评估日期：2026-08-05。M1-M3 已落地后，对原 M4 四项子项逐一审查，基于实际威胁模型重新定级。

#### 评估结论

| 原子项 | 原标记 | 新定级 | 理由 |
|---|---|---|---|
| 频率限流（verify/activate） | M4 可选 | **P1 — 做** | 暴力枚举 license_code、高频激活是真实攻击面。register 限流已在 M1 落地，verify/activate 尚缺 |
| 同 ID 多国家运营告警 | M4 可选 | **P3 — 不做** | 信号无效：只能抓到出差/VPN 的合法用户，抓不到伪造指纹的攻击者（见下方分析） |
| 证书固定 | M4 可选 | **P3 — 暂缓** | 低 ROI：攻击者首选方案是改二进制跳过验证（★难度），不会走 MITM（★★★难度） |
| 不做自动黑名单 | M4 决策 | **✅ 已确认** | 无需实现，维持手动黑名单 |
...
场景                          device_id    告警能抓到吗
──────────────────────────────────────────────────────
合法用户出差（中国→日本）       不变          ✅ 抓到，但这是误报
合法用户用 VPN                不变          ✅ 抓到，但这是误报
攻击者伪造指纹注册新设备        新 ID         ❌ 根本抓不到
克隆机（不改指纹）             不变，共享名额  ❌ 不触发（IP 可能相同）
```

该告警只能产生误报，抓不到目标攻击者。**不做。**
...
- **复用现有 `isD1RateLimited` 机制**（已在 register 路由使用），新增两个 key 前缀：
  - `verify:<license_code>` — 每 license_code 每 60 秒最多 10 次
  - `activate:<license_code>` — 每 license_code 每 60 秒最多 3 次
- **限流行为**：超出返回 `429 Too Many Requests`，body `{ error: "rate_limited", retry_after: 60 }`
- **存储**：沿用 D1 `rate_limits` 表（已在 schema.sql 中），与 register 限流共用
- **副作用安全**：限流仅拒绝请求，不影响已签发的本地 .lic 离线使用（7 天租约内离线可用）

**改动量**：~15 行，在 `drm.ts` 的 verify/activate 路由入口各加一次 `isD1RateLimited` 调用。

**验证标准**：
- 同一 license_code 在 60 秒内超过阈值后返回 429
- 60 秒后请求恢复正常
- register 限流不受影响
...
| 事项 | 决策 | 理由 |
|---|---|---|
| 同 ID 多国家运营告警 | ❌ 不做 | 信号无效，仅产生误报 |
| 证书固定 | ❌ 暂缓 | 低 ROI，攻击者不走这条路 |
| 自动黑名单 | ❌ 不做 | 误伤风险高，维持手动黑名单 |
| 异常检测作为授权阻断 | ❌ 不做 | 克隆已免疫，异常信号区分度不够 |

## 8. 总结

当前本地设备码计算不安全（低熵、可复现、自报无校验）。改为服务端计算/签发后，授权职责拆分为：**指纹（运行时身份）+ max_devices（运行时名额）** 承载判定，**邮箱（账户层）与 device_id（展示层）不参与运行时校验**。免费用户不强制、联网即匿名注册（启动 1 个 register 请求）；付费兑换时云端内联注册并签名绑定；**启动路径每用户每启动至多 1 个 license 请求**——付费为 verify（顺带活跃登记与 device_id 回写）、免费为 register（见 4.1.1）。**复制的虚拟机无需专门对抗**（纯克隆合并占一额；指纹伪造本就无解，防克隆无增量收益，见 3.6.1）。大屏以付费/免费双色展示区间活跃设备（默认最近 1h，数据单源 `device_registry`）。防修改依赖签名与服务端权威化，防伪造依靠限流、黑名单等低成本手段抬高成本，并明确纯软件 DRM 的边界。**M1-M3 已全部落地，M4 仅保留 verify/activate 限流一项（P1），其余原 M4 子项经评估后已关闭。** 订阅续期现状为"一订阅一码自动续费"，新增 Portal 手动续期能力（见 6.6）；退款不封邮箱、未激活退款放行（见 5.4.1）。
