...
- 建议改为**服务端权威登记**：付费兑换时云端计算/签发并下发；免费用户不强制，能联网则匿名注册；启动时注册设备并登记 IP 与活跃状态。
...
2. 按指纹 3 选 2 非空匹配已有设备记录 → 命中则**复用原 device_id**，更新 `last_seen_at/last_ip/经纬度`（**显式刷新，不能沿用 activate 仅新激活写库的时机**，见 4.4 前提）；
...
- 若未来有真实存量：首次注册时按指纹 3 选 2 匹配，用服务端新值回填；过渡期旧 device_id 保留为别名用于黑名单/审计检索。
...
| 里程碑 | 内容 |
| --- | --- |
| M1 | D1 新增 `device_registry` 表 + `register` 端点 + 客户端启动注册（免费匿名、付费复用）；遥测开关与隐私说明就绪；开发期清库重置 `activations` |
...
| M4 | 多 IP/异常频率检测 + 审计 + 黑名单联动 + 限流 + 证书固定（**先只做"同 ID 多国家"告警，不做自动黑名单**） |

---

## 8. 总结

当前本地设备码计算不安全（低熵、可复现、自报无校验）。改为服务端计算/签发后：免费用户不强制、联网即匿名注册；付费兑换时云端下发并签名绑定；启动注册登记 IP 与活跃状态；大屏以付费/免费双色展示区间活跃设备（默认最近 1h）。防修改依赖签名与服务端权威化，防伪造依靠异常检测、黑名单等低成本手段抬高成本，并明确纯软件 DRM 的边界。
...
- **C-3 多 IP 检测误伤**：移动办公/VPN 用户会被"同 ID 多 IP"标记，建议先只做"同 ID 多国家"告警，不做自动黑名单。
