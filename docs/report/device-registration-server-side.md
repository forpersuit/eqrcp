...
> **✅ 实现状态（2026-08-03 完成）**：本节待生效全套机制及服务端双侧预检已完全实现并验证闭环。支持年付与终身全额买断独立隔离、14 天退款期基于 `last_purchased_at` 严格拦截、`auto_renew` 停用防双重扣款、客户端 `/verify` 与 `/activate` 惰性翻转生效，以及 Portal 升级预检接口 `POST /api/v1/user/upgrade-checkout` 与真实 `PRICE_LIFETIME_ID` 挂载。
