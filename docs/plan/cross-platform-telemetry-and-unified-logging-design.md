...
| :--- | :--- | :--- | :--- | :---: |
| **1. `SaveSettings` 仍按旧语义 `SetEnabled(saved.DebugLog\|\|DevMode)`，保存一次设置即整体关闭桌面日志，复现“日志黑洞”** | **【回归·中高危·用户可达】** | 新 FileLogger 恒以 `enabled=true` 启动，但 `app.go:1082 SaveSettings` → `:1093 a.logger.SetEnabled(saved.DebugLog\|\|saved.DevMode)` 未被同步改造。普通用户在 GUI **任意保存一次设置**（DebugLog 未勾）即把 enabled 置 false——`Write`/`enqueue` 整体静默丢弃，agent.log / wails logger / ChatV2 diag / std 日志**全部停写**（含 WARN/ERROR/FATAL），直至重启。这直接违背冻结 §二-3“INFO 基线常态化落盘”与 Phase 1 里程碑“默认常态化落盘”，恰是本方案要消灭的原黑洞场景 | **已闭环建议**：`SaveSettings` 中删除 `SetEnabled(...)` 语义，恒保基线开启，改为 `a.logger.SetDebugMode(saved.DebugLog\|\|saved.DevMode)`；并让 `enqueue`/`Write` 真正以 `debugMode` 门槛过滤 DEBUG/TRACE（现 `debugMode` 字段存而不用，file_logger.go:38/132-143 零消费点，启动后即使开启 DebugLog 也不会提高明细级） | ⏳ **待修复** |
| **2. 饱和紧急兜底在 worker 卡死时会无限阻塞调用协程，违背“绝不挂死”承诺** | **【健壮性·中危】** | `enqueue` 对 WARN/ERROR 先 50ms 有界等待（正确）；超时后 `emergencyDirectWrite` 执行 `l.mu.Lock()`——而恰恰导致 channel 满的原因（worker 持锁在 `file.Sync()`/轮转上遭遇杀软扫描或磁盘 stall）会让该锁**无限不可得**，调用协程在 mu.Lock 上永久阻塞，与冻结 #2“主协程最多仅承受 50ms、绝不挂死”冲突。且紧急直写绕过 `currentSize` 记账与轮转阈值检查，文件可能超 10MB | 紧急兜底改为**独立 O_APPEND 临时句柄**（自开自写自关，不抢 worker 锁）；并把 `file.Sync()` 移出持锁区（写批先释放锁再 sync），从根上缩短锁持有，杜绝长持锁触发该路径 | ⏳ **待收敛** |
| **3. Chat v2 diag 行未被归一、agent.go 门控未按冻结删除（仅靠不变量间接成立）** | **【口径·中危】** | 冻结 #1 要求 `agent.go:996-997` 删除 `fileLogger==nil` 门控并恒定绑定，但 480df074 **未改 agent.go**，现仅凭“GUI 流程 fileLogger 恒非 nil”不变量间接达成（脆弱，未来若拆 CLI/GUI 共用即失效）。且 chat-v2 行经 `MultiWriter` 进入 `Write` 时形如 `chat-v2 2026/09/04 08:08:27 WS ...`——std 前缀检测（index4≠`/`）与 `[` 分支均不命中 → 落 general-text 包为 `[ts][INFO] chat-v2 2026/09/04 ...`（内嵌时间戳重复、无来源框、级别恒 INFO），与冻结 #4“落盘 100% 格式归一、杜绝混排裸行”不符 | (a) 真正执行冻结 #1：删除 `agent.go` 门控，恒定 `srv.ChatV2Logger = diag.NewStdLoggerWithWriter(io.MultiWriter(os.Stderr, agent.fileLogger))`；(b) `Write` 增补对 `chat-v2 `/`2026/.. ` 后缀前缀形态的识别与来源框映射（如 `[CHAT]`），或让 diag 桥接改走 `pkg/logger` 结构化格式 | ⏳ **待收敛** |
| **4. `Tail` 的“线程安全/无裸读”表述过度，轮转窗口内可返回空** | **【健壮性·低危】** | `Tail` 实现为 `os.Open(curPath)` 按路径重开独立句柄（非 Worker 同柄 + 共享读写锁）。轮转发生在“快照路径→Open→扫描”之间时，可能读到刚轮转后的新空 `desktop.log`（GUI 偶发空白）或在行尾撕裂；冻结 #3 要求的“内部同步读、杜绝撕裂竞态”仅部分达成，测试断言过宽（仅 ≤lines/非空）未覆盖“写 + 轮转 + Tail”并发 | `Tail` 与 worker 写/轮转共用同一读写锁：持锁期间从当前句柄整读；或从 worker 侧维护最近 N 行环形缓冲供 `Tail` 无锁快照 | ⏳ **待收敛** |
| **5. `log()` 无条件 `fmt.Println` 刷屏 stdout，单测/无控制台环境噪音巨大** | **【规范·低危】** | `file_logger.go:365-370` `log()` 每次调用同步 `fmt.Println(line)`，即便 enabled=false 也打印；本次 4 项单测即向终端刷出数千行日志，GUI `-H=windowsgui` 下无控制台属纯浪费，且语义上“Write 停写但 Print 仍打”自相矛盾 | `log()` 仅当检测到可挂靠终端/非 windowsgui 时 `fmt.Print`（如 `os.Getenv("EQT_LOG_CONSOLE")` 或直接删除该 stdout 分支，Wails 端已有结构化落盘） | ⏳ **待收敛（可选）** |
| **6. 文档残留名与冻结口径不一致（AsyncFileLogger / SetMinLevel / agent.go 门控示意）** | **【文档·低危】** | §四-3.2 冻结代码块仍写 `fileLogger.SetMinLevel(LogLevelInfo)`（实现中无此 API，实际为 `SetDebugMode`）；§四-4 轮转小节仍称“由 `AsyncFileLogger` 后台单协程调度”；§四-3.2 “Chat 模式贯通修正”展示的 agent.go 无门控代码与真实仓库不符（未删门控） | 正文统一为“保名换芯 FileLogger”口径；§四-3.2 代码块改为真实实现（`NewFileLogger(logPath,true)` + `SetDebugMode(debugLog)` + `log.SetOutput`）；标注 ChatV2Logger 门控当前状态为“仅凭不变量成立，待 #3 收敛删除” | ⏳ **待收敛** |
