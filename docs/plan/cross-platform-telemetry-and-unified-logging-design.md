...
## 十八、第十轮代码复核（commit da5033e4 · §十七 五项闭环核验与并发基准测量口径复核）

> **复核对象**：`da5033e4 test(telemetry): close round-9 review with visual proof in docs/img and tightened CI benchmarks`（v1.36.34，2026-09-04 12:54，对 §十七 五项发现的闭环提交）。
> **复核说明**：核实 §十七 声称闭环的 5 项是否**真实落地**（非文档空转），并对 da5033e4 收紧后的并发基准做测量有效性复核。**本地实测**：`cd desktop/gui && go test -race -run TestTelemetryAndLogging_E2E_FullPipeline -v` → **PASS（0.14s，无竞态）**，并发最大时延实测 15.89ms（-race）。
> **复核结论（总体）**：§十七 五项闭环**全部属实、无虚假声明**——(1) 视觉证据：`docs/img/windows_chrome_telemetry_e2e_test.png` 已入库，`file` 确认真实 PNG（1904×902, 74,260 bytes）；(2) 门禁收紧：并发阈值 `telemetry_e2e_test.go:374` 与单发阈值 `:126` 均改为 ≤100ms，测试日志采用"target <= 50ms, CI gate <= 100ms"双口径；(3) 单发鲁棒：50ms→100ms 消除冷启动抖动；(4) 限流去墙钟：25 顺序→40 协程并发（`sync.WaitGroup`）冲容量 10 桶，毫秒级稳定 ≥30 丢弃触发聚合告警，且 `droppedClientLogCount` 为 `atomic.AddUint64`、令牌桶 `mu sync.Mutex` 串行化（telemetry.go:182/236），并发安全；(5) 权限措辞：补"（由本组件新建目录时生效）"限定。**但本轮发现 1 项并发基准测量口径隐患（中低）**：§12 并发基准与 §6 突发**共享同一 `srv` 与同一 `127.0.0.1` 令牌桶**（Server 级单例、按 `RemoteAddr` 分桶），§6 已把桶打空，§12 的"50 并发最大时延"绝大部分实为 429 快速拒绝路径时延、并非成功摄入路径——详见下表 #1。

### 本轮复核发现

| 评审意见项 | 评审性质 | 复核发现 | 建议处置 | 状态 |
| :--- | :--- | :--- | :--- | :---: |
| **1. §12 并发基准与 §6 突发共享令牌桶，"50 并发最大时延"主要度量 429 拒绝路径而非成功摄入路径** | **【指标·测量有效·中低】** | `telemetry_e2e_test.go` §6（line 170-189）以 40 并发瞬时打空容量 10 / 回填 10 s⁻¹ 的 `127.0.0.1` 桶（~33 丢弃）；§7-§11 文件 I/O（Close、Read、GetLogTail、buildDiagnosticsZip）仅 ms 级、回填 <1 token；§12（line 352-372）50 并发 POST 同一 `srv`、同一 `RemoteAddr`，命中近乎空桶——除极个别外全部 `429`。`postTelemetry` 对 4xx 返回 `err==nil`（line 102-108），`maxLatency` 不区分 204/429 全量计入，故 §12 测得的"50 并发最大时延"实为**限流拒绝路径**时延。更深层：**该基准在物理上无法支撑"50 并发成功摄入"语义**——容量 10 的桶即便满桶也只放行 10/50，剩余 40 必 429。§十六 "50 并发高压力上报，实测最大时延 6.48ms ~ 47.16ms"（line 569）的口径对**并发成功摄入性能**的佐证力弱于字面表述。 | 二选一：(a) §16/§十七 口径收敛为"限流 429 风暴下服务端拒绝路径响应 ≤100ms（CI 门禁）"，去除"50 并发成功上报"含义；(b) §12 改用独立 `server.New`（新监听/新桶，或 `telemetryLimiter` 扩容至 ≥50）真正放行 50 并发，并断言 ≥N 次 204，才能支撑"并发成功摄入 ≤50ms 达标"结论。 | ⏳ 建议跟进（测试测量） |
| **2. 测试头注释残留旧口径** | **【口径·极低】** | `telemetry_e2e_test.go:36` 头注释仍写 "Concurrency & latency performance validation (<= 50ms response time)"，与 da5033e4 已收紧的 ≤100ms 门禁（:374）不一致——双口径对齐未覆盖行首注释。 | 同步为 "(target <= 50ms, CI gate <= 100ms)"。 | ⏳ 可选打磨 |
| **3. 文档 header 状态行口径滞后** | **【口径·极低】** | 文档头"版本"行已声明 "Phase 1~5 全链路落地与端到端联调验收完成"，"状态"行仍写 "Phase 1~4 全部落地…"——两行口径不一致，Phase 5 端到端联调状态未并入状态行。 | "状态"行补 Phase 5 端到端联调验收表述。 | ⏳ 可选打磨 |
