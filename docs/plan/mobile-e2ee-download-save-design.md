...
**独立复核补充说明（记录，不构成本轮整改项）**：
1. **R29 目标路径当前为死代码**：`triggerDownloadItem`（`:1793`，`startE2EEDownload(targetIndex)` 唯一入口）全文件无调用点，单项下载模式实际不可达——R29 修复属预防性且与全批量共用同一映射，未来若接线逐行下载即直接安全；
2. **R28 门控的 `isCompleted` 时序理论边界**：`isCompleted` 亦由状态轮询 `completed` 分支置位（`:2001-2006`，该分支无 `!isE2EEDownloadActive` 守卫，异于进度条分支 `:1913`）。若服务端先于末 chunk 本地解密完成上报 `completed` 且慢解码超出一个轮询周期（1.2s），门控会在解密期提前放开。当前流式模型下末 chunk 拉取即末次解密、间隔远小于轮询周期，无实证窗口；仅作记录——未来若引入「服务端整体推送密文、本地慢速解密」模型，需在此分支补守卫；
3. **`:1274-1279` 空集提示仍复用 `save_done_tips`**：R29 从源头消除幽灵按钮后该分支在正常流中已不可达（条目按钮仅在文件解密完成且有 descriptor 时露出，交付成功即禁用），仅作防御性兜底保留，不另立整改项。
