...
| 审查意见项 | 核心关切与技术风险 | 最终技术决议与落地方案 |
| :--- | :--- | :--- |
| **R26: 移动端多文件 memory 集整批缓冲明文，无聚合上限** | 解密循环的逐文件释放只作用于桌面 UA（`:1686-1693` `!isMobileUA && desc` → assemble→下载→finalize）；移动 UA 按设计在整批解密完成后（`:1732-1748` 露出保存按钮前）把**全部 memory 型 descriptor 及其明文 chunks 留存于内存**，解密期明文峰值 = 本批次所有 memory 文件之和。该累积不受 150MB 聚合预算约束——预算仅作用于交付组装期的 `File[]`（§五矩阵 `MULTI_FILE_SHARE_THRESHOLD` 判定），解密期逐文件 `downloadFile` 只按 `≥256MB && idb` 分流、对 memory 集总量从不设限。父提交前移动端为逐文件 100% 即交付（峰值≈单文件），故这是相对既有行为的退化窗口：多文件集如 N×150~255MB 或大量中小文件聚合超数百 MB 时，iOS Safari 内存压力可在 100% 前杀页，恰为本特性要规避的失败类（R19 只修了桌面端） | **已落实**：采纳「渐进就绪（Progressive Item Readiness）」最佳实践——移动端在解密循环中，单文件解密完成即时激活该条目的【📥 保存】按钮，用户在传输过程中即可边解边存（单项交付即刻 `finalizeDescriptor` 释放该项 chunks 堆内存，将内存峰值从「整批总和」钳制至「在途单项」）；同时对 ≤150MB 小集在 100% 解密后保留【📥 全部保存到手机】批量交付语义。 |

### 第五轮四次复核（整改提交 `874305b2` 落地核对）

> R26 整改提交 `874305b2`（2026-09-02，`download.tmpl.html` +7，仅改模板与本文档，未动版本号）已核对：实现按候选①渐进就绪落地——移动端解密循环内新增 `else if (isMobileUA && desc)` 分支（`:1693-1700`），单文件解密完成即以 `actualFileIdx`（单项模式 = `targetIndex`，全批量 = `fIdx`）`querySelector` 命中对应行按钮并 `display:inline-block`。映射正确性核实无虞：`saveToDevice` 的 index→descriptor 解析为「`filesList[index].name` 或 fileId `f-<index>`」双匹配（`:1258-1267`），对 Map 顺序不敏感、前序条目已被 `finalizeDescriptor` 移除后索引仍稳定，故渐进单项交付与 100% 后的逐条保存均命中正确 descriptor；行序与 `metaData.files` 序对齐前提沿用既有 R16 `f-<idx>` 契约。全批量路径 R26 提前露出的按钮与 100% 完成态的 `itemSaveBtns` 全量显示（`:1754-1755`）互不冲突。`TestTemplateJavaScriptSyntax` 实测通过。决议行已标「已落实」；复核另确认 1 项表述过度承诺，记录为 R27（低危，不阻塞本提交）。

| 审查意见项 | 核心关切与技术风险 | 最终技术决议与落地方案 |
| :--- | :--- | :--- |
| **R27（记录/待文案修正）: R26 决议「内存峰值钳制」为结构性过度承诺** | R26 只解锁了**按钮**，并未解锁**内存**：memory 型 descriptor 的明文 chunks 在用户点击保存并经 `finalizeDescriptor` 前仍整批驻留内存。因此「峰值从整批钳制到在途单项」仅在**用户于解密过程中主动逐项保存**时成立；若用户等到批完成后再保存（或批后段才开始保存），解密期峰值仍 = 全部 memory 之和，iOS Safari 杀页风险原样保留。R26 的真实价值是把「边解边存」从无变为有（能力面改善），不是内存结构保证 | 决议行表述建议降级为「缓解主动交互场景的整批驻留；无交互场景下整批驻留与杀页风险保留」。若未来要求结构性钳制，方向：①解密完成即把 memory 项迁移落 idb（与 R20「idb→引导桌面端」交付语义冲突，须拆分存储模式与交付策略）或②解密期依聚合预算暂停预取并提示（需先预知总量）。**附注低危**：`saveToDevice` 无 pending 分支（`:1274-1279`）空集提示复用 `save_done_tips`（「已保存」）——若条目按钮在清理/停止后残留而再点，会得到误导性成功文案，与 R24 同族提示语问题，建议随文案修正一并改为「无待保存文件」类 in-app 提示 |
