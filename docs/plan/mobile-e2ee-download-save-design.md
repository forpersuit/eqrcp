...
| 审查意见项 | 核心关切与技术风险 | 最终技术决议与落地方案 |
| :--- | :--- | :--- |
| **R26（记录/待决策）: 移动端多文件 memory 集整批缓冲明文，无聚合上限** | 解密循环的逐文件释放只作用于桌面 UA（`:1686-1693` `!isMobileUA && desc` → assemble→下载→finalize）；移动 UA 按设计在整批解密完成后（`:1732-1748` 露出保存按钮前）把**全部 memory 型 descriptor 及其明文 chunks 留存于内存**，解密期明文峰值 = 本批次所有 memory 文件之和。该累积不受 150MB 聚合预算约束——预算仅作用于交付组装期的 `File[]`（§五矩阵 `MULTI_FILE_SHARE_THRESHOLD` 判定），解密期逐文件 `downloadFile` 只按 `≥256MB && idb` 分流、对 memory 集总量从不设限。父提交前移动端为逐文件 100% 即交付（峰值≈单文件），故这是相对既有行为的退化窗口：多文件集如 N×150~255MB 或大量中小文件聚合超数百 MB 时，iOS Safari 内存压力可在 100% 前杀页，恰为本特性要规避的失败类（R19 只修了桌面端） | 需产品/架构决策，本行仅记录候选方向：①**渐进可用**——移动端每个 memory 文件解密完成即注册并即时露出该条目【📥 保存】，峰值钳至「单文件 + 在途」且用户可边解边存（≤150MB 小集保留整批完成语义不变）；②**集聚合预算预分流**——启动时统计 memory 集聚合，超预算提示改用 archive 或桌面端（文案级兜底，不改存储策略，避免与 R20 idb 门控耦合）；③接受并文档化为已知限制（移动端多文件大集建议 desktop/archive）。本轮不改代码，仅记录，待产品决策后再落整改项 |
