...
/**
 * Admin route guard for P2P Signal Worker — Cloudflare Access JWT.
...
export async function requireAdminAuth(
  ...
  if (!accessConfigured(env)) {
    return new Response(
      JSON.stringify({
        error: 'Admin API not configured on P2P Signal Worker (CF_ACCESS_TEAM_DOMAIN / CF_ACCESS_AUD missing)',
  ...
  if (!jwt) {
    return new Response(
      JSON.stringify({
        error: 'Cloudflare Access JWT required for Admin API',
