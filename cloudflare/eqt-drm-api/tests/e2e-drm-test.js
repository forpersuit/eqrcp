...
const DRM_HOSTNAME = process.env.DRM_HOSTNAME || 'lic-test.eqt.net.im';

function execWranglerSQL(sql) {
  const cmd = `CLOUDFLARE_API_TOKEN="" npx wrangler d1 execute ${DB_NAME} --remote --command "${sql.replace(/"/g, '\\"')}"`;
