...
function wrangler(sql) {
  ...
  const out = execSync(
    `CLOUDFLARE_API_TOKEN="" npx wrangler d1 execute ${DB_NAME} --remote --command ${JSON.stringify(oneLine)}`,
...
async function main() {
  ...
  assert(
    ...
    String(rL2.json.error || '').includes('叠加') ||
    String(rL2.json.error || '').includes('绑定'),
    'stack message: ' + JSON.stringify(rL2.json)
  );
