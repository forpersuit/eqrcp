...
const translations = {
    zh: {
        ...
        free_tier_rules_desc: '桌面端 GUI 每日可免费体验 10 分钟全功能的 Share (分享) 与 Receive (接收) 传输（支持移动端待传输列表的自由编辑与累加文件），超时后将触发限制约束并停止传输；Chat 模式每日免费额度为 5 分钟，超额后启用限速等限制。提示：纯命令行终端模式的传输无任何额度与速度约束，但命令行模式下不支持 Chat 对话功能。',
    ...
    en: {
        ...
        free_tier_rules_desc: 'The desktop GUI offers 10 minutes of daily high-speed, full-featured Share & Receive transfers (supporting free editing and file adding on the mobile queue). Transfers will be stopped upon quota exhaustion. Chat mode has a 5-minute daily limit, switching to throttled speed thereafter. Note: Pure CLI terminal transfers are completely unlimited but do not support Chat mode.',
    ...
    ja: {
        ...
        free_tier_rules_desc: 'デスクトップGUIは、毎日10分間の高速かつフル機能のShare＆Receive転送を提供します（モバイル端末でのキュー編集やファイルの追加をサポート）。制限時間を超えると転送は停止します。チャットモードの毎日の無料制限は5分間で、その後は速度制限が適用されます。注意：コマンドライン（CLI）モードでの転送には制限はありませんが、チャットモードはサポートされていません。',
    ...
    ko: {
        ...
        free_tier_rules_desc: '데스크톱 GUI는 매일 10분 동안 고속 및 모든 기능을 지원하는 Share & Receive 전송을 제공합니다(모바일 전송 대기열 편집 및 파일 추가 지원). 시간 초과 시 전송이 차단됩니다. 채팅 모드의 일일 무료 제공량은 5분이며 초과 시 속도 제한이 적용됩니다. 참고: 순수 명령줄(CLI) 터미널 모드의 전송은 아무런 제한이 없으나 채팅 기능은 지원하지 않습니다.',
    ...
    es: {
        ...
        free_tier_rules_desc: 'La GUI de escritorio ofrece 10 minutos diarios de transferencia Share & Receive de alta velocidad y funciones completas (admite edición libre en el móvil). Se detendrán al agotar la cuota. El modo Chat tiene un límite diario de 5 minutos, aplicando restricciones de velocidad después. Nota: Las transferencias en modo CLI puro son ilimitadas pero no admiten chat.',
    ...
    de: {
        ...
        free_tier_rules_desc: 'Die Desktop-GUI bietet täglich 10 Minuten schnellen Share & Receive-Transfer mit vollem Funktionsumfang (unterstützt das Bearbeiten und Hinzufügen auf dem Mobiltelefon). Nach Ablauf wird die Übertragung gestoppt. Der Chat-Modus hat ein Limit von 5 Minuten pro Tag, danach greift eine Drosselung. Hinweis: Der reine CLI-Modus hat keinerlei Einschränkungen, unterstützt jedoch keinen Chat.',
    ...
    fr: {
        ...
        free_tier_rules_desc: 'L\'interface GUI propose 10 minutes par jour de transferts Share & Receive haut débit avec toutes les fonctionnalités (y compris l\'édition libre de la file sur mobile). Les transferts s\'arrêtent après dépassement. Le mode Chat est limité à 5 minutes par jour, puis bridé. Note : Les transferts en CLI pur ne sont pas limités mais ne supportent pas le mode Chat.',
