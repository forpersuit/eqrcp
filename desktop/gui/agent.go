...
func cloneTaskRecord(record TaskRecord) TaskRecord {
	...
	if record.TransferClientStates != nil {
		...
		record.TransferClientStates = cloned
	}
	return record
}
...
func (agent *desktopAgent) runTask(task AgentTask) error {
	...
	if cfg.Secure && !server.GetPaidStatus() && os.Getenv("EQT_TESTING") != "true" {
		...
		cfg.Secure = false
	} else if cfg.Secure && !cert.HasValidCertificateForNode(cfg.TlsCert, cfg.TlsKey, server.GetDeviceNodeID()) {
		agent.log.Infof("runTask: LAN-TLS is enabled in settings, but no valid TLS certificate is available in %s. Gracefully falling back to plain HTTP.", config.DefaultCertsDir())
		cfg.Secure = false
	}
